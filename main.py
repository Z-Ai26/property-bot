...
def is_bare_affirmative(text: str) -> bool:
    ...
    return normalize_text(text) in AFFIRMATIVE_EXACT
...
def is_affirmative_video_request(text: str) -> bool:
    ...
    return bool(
        re.search(
            r"\b(?:send|share|show)\b.*\b(?:video|tour|it)\b",
            norm,
        )
    )
...
def consume_pending_video_reply(
    ...
    if (
        created_at
        and time.time() - created_at > SESSION_TIMEOUT_SECONDS
    ):
        with sessions_lock:
            session = user_sessions.get(sender)
    ...
    def keep_pending() -> None:
        with sessions_lock:
            ...
            if session_inner:
                ...
                state_inner["pending_video_created_at"] = time.time()
...
def format_property_results(
    ...
    lines.append("")
    lines.append(
        f"To lock in the best possible deal, *{AGENT_NAME}* "
        f"({AGENT_PHONE}) will personally negotiate the price and "
...
def build_verified_property_fact_reply(
    ...
    if is_inventory_yes_no_question(user_question):
        ...
        if context:
            return (
                f"Yes — I do have {unit_type} options in *{context}*. "
                "Shall I show you the best ones?"
            )
    ...
    if is_single_availability_question(user_question):
        ...
        if status:
            return (
                ...
                f"For final confirmation, *{AGENT_NAME}* "
                f"({AGENT_PHONE}) can lock it in for you."
            )
    ...
    if len(matches) == 1 and any(
        ...
        if actual and offer and (
            ...
            return (
                ...
                f"best price is 💎 *{offer}*. For negotiation, "
                f"*{AGENT_NAME}* ({AGENT_PHONE}) will get you the "
                "sharpest possible deal."
            )

        if actual:
            return (
                f"The recorded price is *{actual}*. For the latest "
                f"confirmation and the best deal, *{AGENT_NAME}* "
                f"({AGENT_PHONE}) is your man."
            )
...
def create_ai_reply(
    ...
    if property_query or ai_filters:
        ...
        if (
            ...
            reply = (
                "That exact combination has just been taken — the good "
                "ones move fast in Dubai. Would you be open to a "
                "nearby area, a different unit type, or a slightly "
                "adjusted budget? I will find you something you love."
