...
def answer_process_question(
    ...
    STRICT CONTEXT LOCK.
      ...
      STEP A  Is a building/area context active?
      STEP B  If yes, look ONLY inside that context (sheet, then that
              building's knowledge entries, then company-wide policy).
      STEP C  Still nothing? Give a targeted answer for THAT property
              and pivot to the broker. Never substitute another
              building's data.
      STEP D  Only with a completely empty context may the answer be
              global.
    """
    active_context = active_context or {}
    ...
    if verdict == "no":
        # A verified negative is an answer, not a failure. Saying it
        # plainly beats pretending the data is missing.
        return (
            f"No — {detail_label} is not available{scope_note}, "
