...
def consume_pending_video_reply(
    ...
    if len(selected_links) == 1:
        return (
            "Of course — here is the verified video tour:\n\n"
            f"{selected_links[0]['url']}\n\n"
            "Would you like me to arrange a viewing as well?"
        )

    lines = ["Of course — here are the verified video tours you requested:"]
...
def format_property_results(
    ...
    # 🔥 PRO FIX: यहाँ से '[:MAX_PROPERTIES_TO_SHOW]' हटा दिया गया है
    # अब यह कोड उसे नहीं काटेगा, जो लिमिट सेट है वही छापेगा!
    ...
    for building_name, group_records in grouped.items():
        # 🔥 स्मार्ट लॉजिक: अगर नाम में पहले से Building या Tower नहीं है, तभी 'Building' जोड़ेगा
        display_name = building_name
        if display_name != "Matching Property" and "building" not in display_name.lower() and "tower" not in display_name.lower():
    ...
    if video_links:
        if len(video_links) == 1:
            lines.append(
                "\n🎥 I have the layout video for this property.\n"
                "👉 *Type 'YES' to see it.*"
            )
        else:
            lines.append(
                "\n🎥 I have the layout videos for these properties.\n"
    ...
    if has_more:
        lines.append(
            "\n👉 *Type 'MORE' to see other available options.*"
        )
    else:
        lines.append(
...
def select_property_page_memory_fallback(
    ...
    filters: Dict[str, Any] = None,  # 🔥 बस ये लाइन मिसिंग थी!
    search_text: str = "",           # 🔥 और ये लाइन मिसिंग थी!
    ...
    with sessions_lock:
        ...
        for record in records:
            ...
            # 🚀 डायनामिक लिमिट लॉजिक
            filters = filters or {}
            is_bldg = bool(filters.get("building"))
            is_admin = "admin all" in (search_text or "").lower()
...
def select_property_page_memory_fallback(
    ...
    signature: str,
    requesting_more: bool,
    filters: Dict[str, Any] = None,  # 🔥 यहाँ रास्ता खोल दिया
    search_text: str = "",           # 🔥 यहाँ रास्ता खोल दिया
) -> Tuple[List[Dict[str, Any]], bool, bool]:
    records = dedupe_records(records)
    ...
    try:
        ...
        return select_property_page_memory_fallback(
            ...
            filters=filters,         # 🔥 डेटा भेज दिया
            search_text=search_text, # 🔥 डेटा भेज दिया
        )
    ...
    try:
        with seen_lock:
            ...
            for record in records:
                ...
                # 🚀 PRO ARCHITECTURE: Dynamic Limit
                is_bldg = bool(filters.get("building"))
                is_admin = "admin all" in (search_text or "").lower()
        ...
        return select_property_page_memory_fallback(
            ...
            signature=signature,
            requesting_more=requesting_more,
            filters=filters,         # 🔥 बैकअप फंक्शन को डेटा भेजा
            search_text=search_text, # 🔥 बैकअप फंक्शन को डेटा भेजा
        )
    finally:
        if conn is not None:
            conn.close()
...
def create_ai_reply(
    ...
    if matches and not missing_location_context:
        ...
        if fact_reply:
            ...
            visible_matches, exhausted, has_more = ( # 🔥 '_' को हटाकर 'has_more' कर दिया
            ...
            if exhausted:
                ...
                reply = format_property_results(
                    ...
                    filters=effective_filters,
                    user_text=user_question,
                    has_more=has_more, # 🔥 यहाँ 'has_more' पास कर दिया
                )
