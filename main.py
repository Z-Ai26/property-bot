...
def consume_pending_video_reply(
    ...
    with sessions_lock:
        ...
        if len(links) > 1:
            ...
            for item in links:
                ...
                label_tokens = set(
                    meaningful_tokens(label)
                )
                user_tokens = set(
                    meaningful_tokens(text_search)
                )
    ...
    lines = [
        "Of course — here are the verified video tours you requested:"
    ]
    ...
    for index, item in enumerate(
        ...
        label = clean_cell(
            item.get("label", f"Property {index}")
        )
