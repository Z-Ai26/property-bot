...
def parse_money_value(text: Any) -> Optional[float]:
    """
    Parses a money-ish string into a plain float, e.g.:
        "45,000"      -> 45000.0
        "AED 45000"   -> 45000.0
        "45k"         -> 45000.0
        "1.2 lakh"    -> 120000.0
    Returns None if nothing numeric could be extracted. Used both for
    reading price/rent cells from the sheet and for parsing a client's
    stated budget out of free text.
    """
    text = clean_cell(text)
    if not text:
        return None
    ...
    t = text.lower().strip()
    t = re.sub(r"\b(aed|dhs|dirhams?|per\s*year|/\s*year|yearly|annual|only)\b", "", t).strip()

    lakh_match = re.match(r"^([\d.]+)\s*(?:lakh|lac)s?$", t)
    if lakh_match:
        try:
...
def extract_budget_from_text(text: str) -> Dict[str, float]:
    """
    Deterministic fast-path for common, explicit budget phrasings ("under
    70k", "budget is around 50000", "between 40k and 60k", "nothing above
    1 lakh") so these don't need an AI call at all. Casual/implicit budget
    language ("something cheap", "nothing too fancy") is handled separately
    by ai_understand_query.
    """
    norm = normalize_text(text)
    result: Dict[str, float] = {}

    between = re.search(
        r"between\s+([\d.,]+\s*(?:k|lakh|lac)?)\s*(?:and|to|-)\s*([\d.,]+\s*(?:k|lakh|lac)?)",
        norm,
    )
    if between:
    ...
    # "nothing/not/no above X" is a MAX phrased as a negated minimum ("don't
    # show me anything over X") -- must be checked before the plain
    # above/over patterns below, or it would be misread as a min_price.
    negated_max = re.search(
        r"(?:nothing|not|no)\s+(?:above|over|more than)\s*(?:aed|dhs)?\s*([\d.,]+\s*(?:k|lakh|lac)?)",
        norm,
    )
    if negated_max:
    ...
    # Symmetric case: "nothing under X" / "not below X" is a negated MIN.
    negated_min = re.search(
        r"(?:nothing|not|no)\s+(?:under|below|less than)\s*(?:aed|dhs)?\s*([\d.,]+\s*(?:k|lakh|lac)?)",
        norm,
    )
    if negated_min:
    ...
    max_patterns = [
        r"(?:under|below|less than|max(?:imum)?|up to|within)\s*(?:aed|dhs)?\s*([\d.,]+\s*(?:k|lakh|lac)?)",
        r"budget(?:\s+is)?\s*(?:around|about|roughly)?\s*(?:aed|dhs)?\s*[:=]?\s*([\d.,]+\s*(?:k|lakh|lac)?)",
    ]
    ...
    min_patterns = [
        r"(?:above|over|more than|min(?:imum)?|starting from)\s*(?:aed|dhs)?\s*([\d.,]+\s*(?:k|lakh|lac)?)",
    ]
...
def format_property_results(
    ...
    records = records[:MAX_PROPERTIES_TO_SHOW]
    count = len(records)
    ...
    for building_name, group_records in grouped.items():
        ...
        for record in group_records:
            ...
            # Print ALL non-empty sheet columns -- for THIS record only.
            # (Previously this loop was mis-indented one level out, so it ran
            # once per BUILDING using whatever `record` was left over from the
            # last unit in the group -- every unit before the last one showed
            # a title with no details at all.)
            ...
            # Was outside every loop before (ran once, ever) so every listing
            # was numbered "1." -- now increments once per record, as intended.
            global_index += 1

    if total_count > count:
        lines.append(
            f"\nThere are *{total_count - count}* more matching properties I'm "
            "not showing here to keep this readable -- tell me more about what "
            "you need (budget, bedrooms, or move-in date) and I'll narrow it down."
        )
...
def create_ai_reply(sender: str, user_question: str) -> str:
    ...
    # Whole-word check (not substring) so words like "now" or "renovated"
    # don't get mistaken for "no"/"not" and wipe the session unexpectedly.
    ...
    # Deterministic keyword/regex matching found nothing usable. Before
    # giving up, let the AI model interpret the message more broadly --
    # casual phrasing, implied budget, "near the beach" style requests --
    # and map it onto the REAL vocabulary in the sheet. The model only ever
    # returns a filter dict here; the property data shown to the client
    # still always comes from search_properties()/format_property_results()
    # below, so this can't hallucinate a listing that doesn't exist.
    ...
    # ai_filters being non-empty (even with zero matches) is itself strong
    # evidence this was a real property question, so it counts the same as
    # is_likely_property_query() here.
