...
import traceback
import math
import difflib  # NEW -- stdlib, no install needed. Used for typo-tolerant matching.
...
# NEW -- "offer_price" is the discounted / best / current-asking price a
# client is actually quoted, as distinct from "price" (the original/actual
# price). Many Dubai listing sheets name this column "New Rent- <date>",
# which is why "new rent" is the first alias -- the trailing date in a
# real header (e.g. "New Rent- 24-07-2026") still matches it via the
# existing partial/substring matching in find_column().
COLUMN_ALIASES["offer_price"] = [
    "new rent", "offer price", "best price", "discounted price",
    "special price", "final price", "lowest price", "negotiated price",
]
FIELD_EXCLUDES["offer_price"] = ["previous", "annual", "yearly", "monthly"]
...
def resolve_schema(columns: List[str]) -> Dict[str, str]:
    ...
    field_order = [
        ...
        "unit_no",
        "price",
        "offer_price",  # NEW -- see COLUMN_ALIASES["offer_price"] above.
        "size",
        "status",
        "rental_yield",
        "landmark_keywords",
...
# ============================================================
# REFACTORED -- best_value_match() + area/building resolution
# ============================================================
#
# WHY THIS CHANGED:
# The old best_value_match(), in "exact" mode, treated a value as matching
# whenever it was a padded substring of the query:
#       if f" {value_search} " in padded_query: ...
# That is direction-unsafe. For query "Warsan 4" and building value
# "Warsan", " warsan " IS a substring of " warsan 4 ", so the SHORT
# building name "won" purely because it happened to be a text-prefix of
# the longer, unrelated area query. That one line was the root cause of
# bug #1 (area search collapsing onto a same-named building) and, once a
# stray "building" filter leaked in that way, a contributor to bug #3
# (stale context carrying into the next turn).
#
# The fix scores EVERY candidate on a tiered scale (exact > phrase-match
# weighted by length > fuzzy/typo tolerance > token overlap) via the new
# _match_candidates_scored() helper, and -- critically -- the new
# resolve_area_or_building() below runs that same scorer against BOTH the
# building vocabulary and the area/location-keyword vocabulary and keeps
# only whichever one actually explains more of the query. That is what
# lets "Warsan 4" resolve to the AREA and bare "Warsan" resolve to the
# BUILDING, using one consistent mechanism instead of two independent,
# order-dependent checks that could both "succeed" at once.
...
def _match_candidates_scored(
    ...
    """
    Internal: scores every candidate value against the user's free text and
    returns (score, value) pairs, highest first. best_value_match() and
    resolve_area_or_building() both build on this so their scores are
    always directly comparable to one another.

    Tiers (highest wins):
      1. Exact match after normalization -- the strongest possible signal.
      2. The value appears as a complete phrase inside the query
         (word-boundary safe via padding). Longer values score higher, so
         when two candidates both match this way (e.g. building "Warsan"
         and area keyword "Warsan 4" both technically fit inside a query
         like "Warsan 4"), whichever explains MORE of what the client
         actually typed wins.
      3. Fuzzy/typo tolerance (e.g. "7 season" vs "7 SEASONS"), skipped for
         very short values where a fuzzy ratio would be noisy.
      4. Meaningful token overlap -- only for mode in {"partial","location"}
         (e.g. bare "Raffa" should still find area keyword "Al Raffa").
    """
    ...
    query_tokens = set(meaningful_tokens(user_text))
    ...
    for value in values:
        ...
        if value_search == query:
            scored.append((1_000_000 + len(value_search), value_clean))
            continue

        # Tier 2 -- complete phrase contained in the query.
        if f" {value_search} " in padded_query:
            scored.append((100_000 + len(value_search), value_clean))
            continue

        # Tier 3 -- fuzzy typo tolerance.
        if len(value_search) >= 4:
            ratio = difflib.SequenceMatcher(None, value_search, query).ratio()
            if ratio >= 0.87:
                scored.append((90_000 + ratio * 1000, value_clean))
                continue

        # Tier 4 -- meaningful token overlap (area/location-style lookups,
        # or a last-resort building match -- see resolve_area_or_building).
        if mode in {"partial", "location"}:
            ...
            if overlap:
                strong = (
                    len(overlap) >= 2
                    or any(len(token) >= 4 and not token.isdigit() for token in overlap)
                )

                if strong:
                    score = 500 + 100 * len(overlap) + sum(len(token) for token in overlap)
...
def best_value_match(user_text: str, values: List[str], mode: str = "exact") -> str:
    ...
    mode:
    - exact: value must be present as a phrase in the query (plus fuzzy
      typo tolerance).
    - partial/location: also allows token overlap for area/building
      follow-ups.

    (Same signature and return type as before -- every existing call site
    keeps working unchanged.)
    """
    matches = _match_candidates_scored(user_text, values, mode)
...
def get_area_candidate_values(
    ...
    """
    NEW. Area/community candidates a client might type: broad values from
    the LOCATION-style column (e.g. "Dubai", "Sharjah") PLUS every
    individual sub-community pulled out of the Nearby Location / Location
    Keywords column (e.g. "Warsan 4", "Dubai International City", "Al
    Nahda 1"). Both are searched together so a client can type either a
    whole city or a specific neighbourhood and be understood the same way.

    This matters because on a typical Dubai sheet the plain "Location"
    column is often just the city ("Dubai"/"Sharjah"); the actual
    sub-community names live in Location Keywords. Only searching the
    former (the old behaviour) meant area names almost never matched at
    all -- which is the deeper reason bug #1 could happen: with location
    detection essentially never succeeding, only the (buggy) building
    substring check was ever left to "explain" an area query.
    """
    values: List[str] = []
    seen = set()

    location_column = schema.get("location", "")
    landmark_column = schema.get("landmark_keywords", "")

    for column in (location_column, landmark_column):
        if not column:
            continue

        for value in get_unique_column_values(records, column, split_values=True):
...
def resolve_area_or_building(
    ...
    """
    NEW. Decides whether this message is naming an AREA (broad discovery --
    diverse buildings) or a specific BUILDING (deep dive -- many units in
    one building), by scoring the message against BOTH vocabularies with
    _match_candidates_scored() and keeping only the stronger match.

    Returns at most one of {"location": ...} or {"building": ...} -- never
    both -- so search_properties() can branch cleanly on which mode this
    is, and so a weaker/spurious match from the losing category never
    leaks into the filters. Building wins ties, matching the requirement
    that a specific building name takes priority over an area filter.
    """
    building_column = schema.get("building", "")

    area_values = get_area_candidate_values(records, schema)
    building_values = (
        get_unique_column_values(records, building_column, split_values=False)
        if building_column else []
    )

    area_matches = _match_candidates_scored(user_text, area_values, mode="location")
    building_matches = _match_candidates_scored(user_text, building_values, mode="exact")

    # Last-resort loose building lookup (e.g. "Fardan" -> "AL FARDAN"),
    # matching the original behaviour: only when NEITHER vocabulary found
    # anything yet.
    if not building_matches and not area_matches and building_column:
        building_matches = _match_candidates_scored(user_text, building_values, mode="partial")

    best_area = area_matches[0] if area_matches else None
    best_building = building_matches[0] if building_matches else None

    if best_building and (not best_area or best_building[0] >= best_area[0]):
...
def extract_filters_from_text(
    ...
    # REFACTORED -- area vs. building used to be two independent checks
    # (one against the location column, one against the building column)
    # that could both "succeed" on the same message. resolve_area_or_building()
    # scores the message against both vocabularies at once and returns
    # only the stronger match, so a message is always cleanly classified
    # as an area search OR a building search -- never both, and never the
    # wrong one just because a short building name happened to be a
    # text-prefix of a longer area query. See its docstring above.
    filters.update(resolve_area_or_building(user_text, records, schema))
...
def build_effective_filters(
    ...
    REFACTORED. Relies on an invariant established by the new
    extract_filters_from_text()/resolve_area_or_building(): a single
    message never sets BOTH "location" and "building" -- exactly one, or
    neither. That means whenever either is freshly stated this turn, it is
    an unambiguous new area/building the client just typed, and it always
    fully REPLACES whatever area/building context was remembered from
    before (building is checked first, so a freshly-named building always
    wins priority over a remembered -- or even freshly-named -- area, per
    the "building search prioritized over area search" requirement). This
    is what stops the bot from mixing an old "Warsan" search into a later
    "Al Nahda" query: there is no merge step where a stale value could
    ...
    if current_building:
        # A specific building was just named -- deep-dive mode. This takes
        # priority over any area/location context (new or remembered);
        # only the unit type (if also stated this turn) rides along.
        ...
        # A specific area was just named -- broad discovery mode. Same
        # full replacement of old location/building context.
        ...
        # Mutually exclusive, mirroring the invariant above: previous
        # filters never hold both location and building at once either,
        # since they were saved from a prior call to this same function.
...
def unit_type_matches_record(
    ...
    if desired.lower() == "studio":
        if "studio" in combined_search:
            return True
        # SMALL BONUS FIX: Dubai listing sheets very commonly abbreviate
        # Studio as "STU" in the unit-type column itself, with the word
        # "studio" not appearing anywhere else on the row. Without this,
        # a client typing "Studio" matched zero rows on sheets that use
        # that shorthand -- recognize it too.
        if unit_type_column:
...
def apply_hard_filters(
    ...
    if location:
        candidates = [
            ...
            or (
                landmark_column and phrase_in_text(location, record.get(landmark_column, ""))
            )
            or phrase_in_text(location, row_search_text(record))
        ]

    # REFACTORED: match the building filter ONLY against the building
    # column when it's known -- not the whole row's text. A building's own
    # Location Keywords cell can mention its area in a way that looks like
    # another building's name (e.g. building "WARSAN" and building
    # "7 SEASONS" are both in the "Warsan 4" area, so "7 SEASONS"'s own
    # keywords cell contains the word "Warsan"). Falling back to row-wide
    # text here meant filtering by building="WARSAN" could also pull in
    # 7 SEASONS rows just because THEIR keywords happened to say "Warsan
    # 4" too -- silently reintroducing bug #1/#2 at the filtering layer
    # even after classification was fixed. Only fall back to row-wide text
    # if the building column itself could not be identified at all.
    if building:
        if building_column:
            candidates = [
                record for record in candidates
                if phrase_in_text(building, record.get(building_column, ""))
            ]
        else:
            candidates = [
                record for record in candidates
                if phrase_in_text(building, row_search_text(record))
            ]
...
# ============================================================
# REFACTORED -- search_properties() + one-per-building diversity helpers
# ============================================================
#
# WHY THIS CHANGED:
# The old function returned a flat list in whatever order the sheet rows
# happened to be in. Slicing that to MAX_PROPERTIES_TO_SHOW meant an area
# search with several matching buildings could easily show 4 units from
# the FIRST matching building and never even reach the second one (bug #1's
# other half -- fixing the filter alone isn't enough without this). And a
# building-specific search had no special handling at all to make sure it
# showed *multiple* units from that one building (bug #2).
#
# The fix adds two small helpers -- sort_records_by_value() (best/offer
# price first) and pick_one_per_building() (exactly one -- the best-priced
# -- unit per building) -- and applies them as a single, consistent
# finishing step: a specific building filter ("building_mode") skips the
# one-per-building step and just sorts by price so the client sees several
# real options inside that one building; everything else (area search,
# unit-type-only follow-ups, even the last-resort free-text fallback) gets
# the diversity pass so a broad search always showcases variety across
# buildings instead of repeating one building several times.
...
def sort_records_by_value(
    ...
    """
    NEW. Best-priced first -- prefers the offer/best price when present,
    falling back to the actual price. Records with no parseable price at
    all keep their original relative order, at the end (never dropped).
    """
    def sort_key(record: Dict[str, Any]) -> Tuple[int, float]:
        value = to_optional_float(get_record_value_by_field(record, schema, "offer_price"))
        if value is None:
            value = to_optional_float(get_record_value_by_field(record, schema, "price"))
        if value is None:
            return (1, 0.0)
        return (0, value)
...
def pick_one_per_building(
    records: List[Dict[str, Any]],
    schema: Dict[str, str],
) -> List[Dict[str, Any]]:
    """
    NEW. Keeps exactly one -- the best-priced -- unit per unique building,
    in the order buildings first appear. Used for area/broad searches so a
    client sees variety across the neighbourhood instead of several units
    piled up from a single building.
    """
    building_column = schema.get("building", "")
    grouped: "OrderedDict[str, List[Dict[str, Any]]]" = OrderedDict()

    for record in records:
        name = clean_cell(record.get(building_column, "")) if building_column else ""
        grouped.setdefault(name or "__unknown__", []).append(record)

    return [sort_records_by_value(group, schema)[0] for group in grouped.values()]
...
def search_properties(
    ...
    # A specific building was named -- deep-dive mode. Everything else
    # (including a bare area/location filter) gets the one-per-building
    # diversity pass at the end instead.
    ...
    # ---- Diversity / ranking finishing pass (see module comment above). --
    if building_mode:
        results = sort_records_by_value(results, schema)
    else:
        results = pick_one_per_building(results, schema)
...
# ============================================================
# REFACTORED -- format_property_results()
# ============================================================
#
# WHY THIS CHANGED:
# 1. The Nearby Location / Location Keywords column was in
#    CLIENT_HIDDEN_COLUMN_KEYWORDS, so it was never shown to the client at
#    all, even though it's exactly the "Warsan 4, Dubai International
#    City"-style sub-community detail the client needs to see. It's left
#    hidden from the generic column dump below (still correct there, so it
#    doesn't get shown twice) and instead surfaced explicitly, once per
#    building group, cleaned up via the new format_nearby_location_for_display().
# 2. "Actual Price" and the offer/best price used to show up wherever they
#    happened to fall in the generic column loop -- correct only by
#    accident (the "new rent" text match), and not guaranteed to sit next
#    to each other. They're now always-present, explicitly labeled lines.
# 3. The intro line now recognizes area-search vs. building-search mode
#    (via the same filters dict search_properties() already branches on)
#    for a brief, friendlier explanation of what's being shown, per the
#    "Here is a great unit in the 7 Seasons building, located in Warsan
#    4..." example in the spec.

def format_nearby_location_for_display(raw_value: str, max_parts: int = 4) -> str:
    """
    NEW. Turns a raw, possibly multi-value Nearby Location / Location
    Keywords cell (e.g. "International City Phase 2/Warsan 4/Dubai
    International City") into a short, clean, comma-separated line for the
    client, de-duplicating near-identical fragments (e.g. "DIC" / "Dic")
    and capping the length so one property's location line never runs on.
    """
...
def format_property_results(
    ...
    building_column = schema.get("building", "")
    is_building_search = bool(filters.get("building"))
    is_area_search = bool(filters.get("location")) and not is_building_search

# ---- Friendly, mode-aware intro. ----------
    if is_building_search:
        intro = f"Sure — here's what's available in *{filters['building']}* right now:"
    elif is_area_search:
        intro = f"Sure — here's a great selection across *{filters['location']}*:"
    else:
        filter_description = describe_filters(filters)
        intro = "Sure — here are the best matching options"
        if filter_description:
            intro += f" for *{filter_description}*"
        intro += ":"

    # 🌟 यहाँ से हमने Total Count और Top 4 वाला पूरा सिस्टम हटा दिया
    ...
    for building_name, group_records in grouped.items():
        ...
        if len(group_records) > 1:
            building_header += f" — {len(group_records)} available units"

        lines.append(building_header)

        # Exact sub-community / nearby location, pulled from the Location
        # Keywords column (falls back to building+location if a row has no
        # keywords set at all). Shown once per building group since it's
        # the same for every unit inside that building.
        nearby_raw = get_record_landmark_keywords(group_records[0], schema)
        nearby_display = format_nearby_location_for_display(nearby_raw)

        if nearby_display:
            lines.append(f"📍 *Area:* {nearby_display}")
        ...
        for record in group_records:
            unit_type_value = get_record_value_by_field(record, schema, "unit_type")
            unit_no_value = get_record_value_by_field(record, schema, "unit_no")
            actual_price = get_record_value_by_field(record, schema, "price")
            offer_price = get_record_value_by_field(record, schema, "offer_price")
            size_value = get_record_value_by_field(record, schema, "size")

            title_parts = [
                part for part in [
                    unit_type_value,
                    f"Unit {unit_no_value}" if unit_no_value else "",
                ]
                if part
            ]
            ...
            if unit_type_value:
                lines.append(f"   • *Unit Type:* {unit_type_value}")

            if actual_price:
                lines.append(f"   • *Actual Price:* {actual_price}")

            # Only show the offer/best price as a separate line when it
            # actually differs from the actual price -- otherwise it's
            # just noise repeating the same figure.
            if offer_price and searchable_text(offer_price) != searchable_text(actual_price):
                lines.append(f"   • 💎 *Best Price:* {offer_price}")

            if size_value and parse_float_cell(size_value) not in (None, 0.0):
                lines.append(f"  📏 *Size:* {size_value} Sq.Ft.")

            if unit_no_value:
                lines.append(f"   • *Unit No:* {unit_no_value}")

            global_index += 1

    if total_count > count:
        lines.append("\n🌟 *These are the best options I am showing to you.*")
...
def create_ai_reply(sender: str, user_question: str) -> str:
    ...
    # --- 2. THE ULTIMATE EXPERT AI ROUTING LOGIC (PRO-LEVEL) ---
    if matches:
    ...
    # --- 2. THE ULTIMATE EXPERT AI ROUTING LOGIC (100% BUG-FREE SUPER CODE) ---
    if matches:
        deduped_matches = dedupe_records(matches)
        total_matches = len(deduped_matches)
        
        user_msg = user_question.lower().strip()

        # 🔒 VIP Admin Triggers (सिर्फ आपके लिए)
        is_admin_all = "hi i am zahid show me all record" in user_msg
        is_admin_more = "zahid show more" in user_msg
        
        # 👤 Client Trigger (क्लाइंट के लिए स्मार्ट पेजिनेशन)
        is_client_more = any(word in user_msg for word in ["more", "next", "aur dikhao", "aage", "show more", "another"])

        last_idx = previous_state.get("last_shown_index", 0)

        if is_admin_all:
            # एडमिन: एक साथ पूरा खजाना
            start_idx = 0
            end_idx = total_matches
        elif is_admin_more:
            # एडमिन: अनलिमिटेड आगे बढ़ना
            start_idx = last_idx if last_idx < total_matches else 0
            end_idx = start_idx + MAX_PROPERTIES_TO_SHOW
        elif is_client_more:
            # क्लाइंट: यूनिट 1-4, फिर 5-8, और फिर वापस 1-4
            if last_idx == MAX_PROPERTIES_TO_SHOW:
                start_idx = MAX_PROPERTIES_TO_SHOW
            else:
                start_idx = 0
            ...
            # नया सवाल: हमेशा शुरुआत के 4
            start_idx = 0
            end_idx = MAX_PROPERTIES_TO_SHOW

        visible_matches = deduped_matches[start_idx:end_idx]
        effective_filters["last_shown_index"] = end_idx
        
        set_pending_video_links_for_records(sender, visible_matches, schema, columns)
        
        # 🚀 MAGIC FIX: सिर्फ visible_matches प्रिंट होंगे, कोई रिपीटीशन नहीं!
        property_text = format_property_results(
        ...
        # 🌟 FORCED HIDDEN INVENTORY FOOTER (क्लाइंट को गिनती नहीं दिखेगी)
        if not is_admin_all and total_matches > len(visible_matches):
            if "These are the best options" not in property_text:
                property_text += "\n\n🌟 *These are the best options I am showing to you.*"
        
        # 🧠 DYNAMIC INVENTORY CONTEXT FOR AI (AI को बैकग्राउंड में सब पता होगा)
        unit_counts = {}
        for r in deduped_matches:
            u = get_record_value_by_field(r, schema, "unit_type") or "Property"
            unit_counts[u] = unit_counts.get(u, 0) + 1
        filtered_inventory = ", ".join([f"{c}x {u}" for u, c in unit_counts.items()])
        
        b_name = effective_filters.get("building") or get_record_value_by_field(matches[0], schema, "building") or "The property"
        raw_keywords = get_record_landmark_keywords(matches[0], schema)
        
        loc_parts = [p.strip() for p in re.split(r"[/,\|]", raw_keywords) if p.strip()]
        primary_loc = loc_parts[0] if loc_parts else (get_record_value_by_field(matches[0], schema, "location") or "Dubai")
        nearby_locs = loc_parts[1:] if loc_parts else []
        nearby_text = ", ".join(nearby_locs)
        
        building_context = ""
        if b_name and records:
            b_col = schema.get("building", "")
            b_records = [r for r in records if phrase_in_text(b_name, r.get(b_col, "") if b_col else row_search_text(r))]
            b_counts = {}
            status_col = schema.get("status", "")
            for r in b_records:
                status_val = r.get(status_col, "") if status_col else ""
                if status_is_available(status_val):
                    u = get_record_value_by_field(r, schema, "unit_type") or "Property"
                    b_counts[u] = b_counts.get(u, 0) + 1
            b_inv = ", ".join([f"{c}x {u}" for u, c in b_counts.items()])
            building_context = f"Total available inventory in {b_name} building: {b_inv}."
            
        # 👑 THE MASTER PROMPT (AI Control)
        ai_prompt = (
            f"User's message: {user_question}\n\n"
            f"[System Note for AI: The database found {total_matches} properties for this request.\n"
            f"Primary Location: {primary_loc}\n"
            f"Nearby Locations: {nearby_text if nearby_text else 'None'}\n"
            f"Inventory matching exactly what they asked: {filtered_inventory}.\n"
            f"{building_context}\n\n"
            "Instructions:\n"
            f"1. If the user asks where the building/property is, answer EXACTLY: '{b_name} is situated in {primary_loc}. "
            f"{'Nearby locations are ' + nearby_text + '.' if nearby_text else ''}' Do NOT dump the list.\n"
            "2. If the user asks about inventory (e.g., 'Do you have Studios?'), answer them naturally using the 'Total available inventory' info above.\n"
            "3. To display the property list, output [SHOW_PROPERTIES] anywhere in your response.\n"
            "4. If the user is simply requesting properties (e.g., 'show me', 'more', 'next'), output [SHOW_PROPERTIES] by itself.\n"
            "5. [SHOW_PROPERTIES] automatically appends the formatted list. DO NOT write an intro like 'Here is the list:' if you use it.]"
        )
        
        ai_response = create_general_ai_reply(ai_prompt, past_history, records, schema)
        
        if "[SHOW_PROPERTIES]" in ai_response:
            if ai_response.strip() == "[SHOW_PROPERTIES]":
                reply = property_text
            else:
                reply = ai_response.replace("[SHOW_PROPERTIES]", "\n\n" + property_text).strip()
        else:
            reply = ai_response

        filters_to_save = effective_filters
        text_to_save = search_text or user_question
        matched_status = True

    else:
        # --- 3. ZERO MATCHES / GENERAL CHAT LOGIC ---
        is_prop_query = is_likely_property_query(user_question, effective_filters or current_filters, previous_state) or ai_filters
        ...
        if is_prop_query:
            ...
            missing_info = []
            if not effective_filters.get("location") and not effective_filters.get("building"):
                missing_info.append("location/building")
            
            if missing_info:
                ai_prompt = (
                    f"{user_question}\n\n"
                    "[System Note for AI: The user is asking for a property but forgot to specify the area or building. "
                    "Do NOT say you couldn't find a match. Instead, warmly and politely ask them which specific area or building they are interested in.]"
                )
            else:
                ai_prompt = (
                    f"{user_question}\n\n"
                    "[System Note for AI: An internal search was just completed, but ZERO matching properties were found for this exact request. "
                    "Inform the user politely that we don't have exact matches right now. Ask if they are open to other areas, or refer them to Mr. Zahid at +971562625777.]"
                )
        else:
            filters_to_save = current_filters
            text_to_save = user_question
            ai_prompt = user_question

        reply = create_general_ai_reply(ai_prompt, past_history, records, schema)
