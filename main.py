...
def format_property_results(
    ...
    if is_building_search:
        ...
        intro = "Sure — I found a match" if total_count == 1 else "Sure — I found some matches"
        if filter_description:
            intro += f" for *{filter_description}*"

    intro += f": *{total_count}* matching record" + ("" if total_count == 1 else "s") + "."

    if total_count > count:
        intro += f"\n\nHere are the top *{count}*:"
    elif is_area_search and count > 1:
        intro += f"\n\nHere is one great option in each of the *{count}* buildings I found there:"
    else:
    ...
    for building_name, group_records in grouped.items():
        ...
        for record in group_records:
            ...
            if size_value and parse_float_cell(size_value) not in (None, 0.0):
                lines.append(f"   • *Size:* {size_value} sqft")
...
def create_ai_reply(sender: str, user_question: str) -> str:
    ...
    if matches:
        ...
        if loc_parts:
            ...
            nearby_locs = loc_parts[1:]
        else:
            primary_loc = get_record_value_by_field(matches[0], schema, "location") or "Dubai"
            nearby_locs = []
        ...
        ai_prompt = (
            ...
            f"[System Note for AI: The database found {len(matches)} properties for this request.\n"
            ...
            f"{building_context}\n\n"
            "Instructions:\n"
            f"1. If the user asks where the building/property is (e.g., 'Where is it?', 'Location?'), answer EXACTLY using this sentence structure: '{b_name} is situated in {primary_loc}. "
            f"{'Nearby locations are ' + nearby_text + '.' if nearby_text else ''}' Do NOT dump the property list.\n"
            "2. If the user asks about inventory (e.g., 'Only two?', 'Do you have Studios?'), answer them naturally using the 'Total available inventory' info above.\n"
            "3. If you want the system to display the list of matching properties to the user, include the exact keyword [SHOW_PROPERTIES] anywhere in your response.\n"
            "4. If the user is simply requesting properties (e.g., 'Show me 1BR', 'Need flat', 'Yes show me'), just output [SHOW_PROPERTIES] by itself.\n"
            "5. The [SHOW_PROPERTIES] keyword automatically appends the formatted property list. DO NOT write your own intro (like 'Here is the list:') if you use [SHOW_PROPERTIES].]"
        )
    ...
    # ============================================================
    # THE EXPERT AI ROUTING LOGIC (100% BULLETPROOF)
    # ============================================================
    is_prop_query = is_likely_property_query(user_question, effective_filters or current_filters, previous_state) or ai_filters
    
    if is_prop_query:
        # Clear stale video links since no new property was found
        set_pending_video_links(sender, [])
        filters_to_save = effective_filters
        text_to_save = search_text or user_question
        
        # --- HIDDEN CONTEXT INJECTION FOR OPENAI ---
        missing_info = []
        if not effective_filters.get("location") and not effective_filters.get("building"):
            missing_info.append("location/building")
        
        if missing_info:
            # Case 1: Query is incomplete (e.g., "Need apartment")
            ai_prompt = (
                f"{user_question}\n\n"
                "[System Note for AI: The user is asking for a property but forgot to specify the area or building. "
                "Do NOT say you couldn't find a match. Instead, politely and warmly ask them which specific area or building they are interested in.]"
            )
        else:
            # Case 2: Query is specific, but zero results in the Google Sheet
            ai_prompt = (
                f"{user_question}\n\n"
                "[System Note for AI: An internal database search was just completed for this exact requirement, but ZERO matching properties were found. "
                "Inform the user politely that we don't have exact matches available right now. Ask if they are open to other areas, or refer them to Mr. Zahid at +971562625777.]"
            )
    else:
        # Case 3: Normal conversation (e.g., "Hi", "How are you", "What areas do you cover?")
        filters_to_save = current_filters
        text_to_save = user_question
        ai_prompt = user_question

    # Send the context-aware prompt to OpenAI
    reply = create_general_ai_reply(ai_prompt, past_history, records, schema)
    
    # Session Update: Save the ORIGINAL user_question so the chat history remains clean
    update_session(
        sender=sender,
        user_text=user_question,
        assistant_text=reply,
