...
def ask_consultant(
    ...
    if not client:
        fallback = (
            ...
            "For assistance, please connect with Mr. Zahid at "
            "+971562625777."
        )
    ...
    for _ in range(max_tool_rounds):
        ...
        for item in tool_calls:
            ...
            try:
                ...
                print(
                    f"Tool execution error for {item.name}:",
                    error,
                )
                traceback.print_exc()
                result = {
                    "error": (
                        f"{item.name} failed. Please contact "
                        "Mr. Zahid for assistance."
                    )
                }

            if (
                item.name == "search_listings"
                and isinstance(result, list)
            ):
                for listing in result:
                    video_link = clean_cell(
                        listing.get("video_link", "")
                    )

                    for url in extract_video_urls(
                        video_link
                    ):
    ...
    fallback = (
        ...
        "For immediate assistance, please connect with Mr. Zahid "
        "at +971562625777."
    )
