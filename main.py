...
def search_properties(
    ...
    # Handle follow-ups where only unit type is mentioned, but memory holds raw context
    only_unit_followup = (
    ...
    # 1. Check if the user specifically asked for location, building, or unit type
    specific_filter_present = has_specific_property_filter(filters)

    if specific_filter_present or filters.get("available_only"):
        # Apply STRICT (AND) logic: Must match BOTH Location and Unit Type if provided
        hard_filtered = apply_hard_filters(records, filters, schema)

        # If we found exact matches using strict logic, return them immediately
        if hard_filtered:
            return dedupe_records(hard_filtered)

    # 2. THE SMART FALLBACK: If strict match fails, score the entire message context
    fallback_results = score_search_records(fallback_search_text, records)

    # 3. Strictly enforce the Unit Type (e.g., 1 BR) in the fallback results
    ...
    # 4. Strictly enforce availability if requested
    if filters.get("available_only"):
        fallback_results = filter_available_records(fallback_results, schema)

    # 5. Return ONLY the Top 4 closest matches to prevent overwhelming the user
    return dedupe_records(fallback_results)[:4]
