...
ENABLE_RESPONSES_CONSULTANT = (
    os.getenv("ENABLE_RESPONSES_CONSULTANT", "true").lower() == "true"
)
...
def find_knowledge_snippets(
    ...
    """
    Scores approved company knowledge against the client's words. Only
    text that already exists in knowledge.json is ever returned.
    """
    knowledge = get_knowledge()
...
def answer_process_question(
    ...
    """
    STEP 1 - check the listing sheet for this exact attribute.
    STEP 2 - if it is there, answer it confidently and close.
    STEP 3 - if it is not, use the approved pivot. Never guess.
    """
    listing_value = find_listing_attribute(
        context_records or [],
        columns or [],
        topic,
    )
    ...
    if listing_value:
        scope = f" for *{context_label}*" if context_label else ""

        return (
            f"Yes — confirmed{scope} 👇\n\n"
            f"• {listing_value}\n\n"
    ...
    snippets = find_knowledge_snippets(user_question, topic)
    ...
    return unverified_detail_pivot(
        ATTRIBUTE_DETAIL_LABELS.get(topic, "that")
    )
...
def find_listing_attribute(
    ...
    """
    Returns a verified sheet value for the attribute, or "". Only
    reports a value every matched unit agrees on, so one unit's
    "Chiller Free" is never presented as the whole building's.
    """
    ...
    for column in columns:
        ...
        if any(
            searchable_text(value) != searchable_text(first)
            for value in values[1:]
        ):
            continue

        return f"{clean_cell(column)}: {first}"

    # Second pass: the value may carry the attribute even when the
    # header does not -- "Key Features: Chiller Free, 1 Covered
    # Parking". Word-boundary matched so "riverview" is not a view.
    for column in columns:
        if column == INTERNAL_SEARCH_KEY:
            continue
        ...
        haystack = searchable_text(first)

        if any(
            re.search(
                r"\b" + re.escape(keyword) + r"\w*",
                haystack,
            )
...
def handle_direct_intent(
    ...
    if intent == INTENT_GREETING:
        ...
        return pick_variant(
            ...
            "greeting",
            [
                "Hello, and welcome 😊 I am the property consultant for "
                "The Property Panda.\n\nTell me the area or building "
                "you have in mind — or just the unit type and budget — "
                "and I will pull up the best verified options for you.",
                "Hi there 😊 Lovely to hear from you.\n\nWhich area or "
                "building are you looking at? Give me that and I will "
                "show you exactly what is available right now.",
                "Welcome 😊 I look after verified listings for The "
                "Property Panda.\n\nWhat are you searching for — an "
                "area, a specific building, or a unit type and budget?",
            ],
            last_text,
        )
