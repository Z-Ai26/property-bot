...
v16 - fixes found by running the bot against the live 152-row sheet.

  1. FILLER DUMP (the expensive one). DEFAULT_AVAILABLE_ONLY put
     available_only into every filter set, so search_properties had
     nothing to narrow by and returned the whole inventory. "ok", "hmm"
     and "?" each rendered 4 buildings and burned 4 of the 7 session
     slots, after which a real search answered "these are the best
     options I have" against 119 vacant units. 11/11 fillers dumped
     before; 0/11 now.
  2. UNIT TYPE matched against the whole row, so "Offers studio, 1BR and
     2BR apartments" sitting in Key Highlights made 14 studios answer a
     2BR search. The unit type column is now authoritative.
  3. Unit numbers and sizes printed as "Unit 2209.0" / "1691.0 Sq.Ft."
     on 95 and 99 of 119 live units.
  4. "Actual Price: -" reached clients on 42 units; "63k/65K" on 45.
  5. Every area search folded behind WhatsApp's "Read more" (871-1325
     chars against a ~700 fold). Each building is now its own message.
  6. The pivot read LOCATION, which holds only DUBAI/SHARJAH, so it
     offered Burjuman stock "right next door" to an Al Qusais building.
  7. Budget and unit-type leads were answered with "which area?" and no
     stock. They now see matching units, or an honest out-of-stock line.
  8. A bare "45k" was not read as a budget.
  9. DEFAULT_AVAILABLE_ONLY now defaults true, so booked units cannot
     return because an environment variable went missing.
 10. Listing card redesigned: AED 43,000 not 43k, Studio not STU,
     one area and one metro line instead of four raw keywords.

v16.1 - from a real client transcript.

 11. "How to book" returned the same listing card again. "book" was in
     looks_like_followup's word list, so a question about the process was
     read as "show me that again", and there was no booking answer to give
     anyway. There is one now, assembled from knowledge.json - cheques,
     deposit, commission, Ejari, move-in - so it stays true when the
     policies change. It answers inside a building lock too, since the
     process is the same in every building.
 12. Card shows ONE price. The struck-out "Listed" line invited the client
     to open the negotiation below the offer, and "You Save" argued the
     broker's case before the client asked. The direct price answer was
     aligned to match.
 13. Building name reads "DOTCOM Building"; the metro line reads "Near
     Al Ghubaiba Metro Station" rather than implying the building is the
     station.
 14. Closing lines no longer promise an outcome ("absolutely no issues
     living here") or lean on scarcity ("rents out very fast", "moves
     quickly") - the urgency language that was removed from the spec but
     had stayed in the code.

v17 - the conversation layer (agent.py).

 Measured on 50 real client messages, this engine answered a SEARCH at
 12/12 and a CONVERSATION at 0/5 on viewing requests, 0/5 on "not
 interested" / "yes interested", 43% on building features - 28/50 overall.
 Every failure was the same shape: a regex decided what the client meant.
 "price kya hai" worked and "kya rate hai" repeated the listing, because
 one phrasing was in a keyword list and the other was not. There is always
 another phrasing, so the fix is structural, not another pattern.

 15. message_is_a_property_search() splits the traffic by measured
     strength. A message naming a real area, building, unit type or budget
     goes to this engine, unchanged and instant. Everything else goes to
     agent.py. On 24 real messages the split was right 24 times, typos
     included.
 16. agent.py gives Claude the same Python functions as tools -
     search_properties, get_policy, get_building_facts, request_callback.
     It never receives the sheet, only tool results.
 17. verify_reply() re-checks every AED figure, size, unit number and
     shouted name in the draft against the tool results of that same turn,
     BEFORE the message is sent. One that does not match discards the whole
     reply. Tested against a deliberately lying model: 12/12 inventions
     blocked, 0 reached the client.
 18. Every failure path - no key, no package, API down, empty response,
     failed verification - returns "" and the levels below run exactly as
     they do today. Tested with the model crashing and returning nothing:
     0 empty replies out of 24.
...
# 3, not 4. Each building is now its own WhatsApp message, so 4 meant
# five notifications on one search. Three cards plus a closing reads as
# a curated shortlist rather than a dump.
...
# The WhatsApp number that receives hot-lead alerts. Digits only,
# country code first, no plus sign -- Meta rejects the plus.
AGENT_ALERT_NUMBER = re.sub(
    r"[^0-9]",
    "",
    os.getenv("AGENT_ALERT_NUMBER", "971562625777"),
)

ENABLE_LEAD_ALERTS = (
    os.getenv("ENABLE_LEAD_ALERTS", "true").lower() == "true"
)

# Meta only permits free-form messages inside a 24-hour window
# opened by the RECIPIENT's last inbound message. Set this to an
# approved template name to alert the broker outside that window.
AGENT_ALERT_TEMPLATE_NAME = os.getenv("AGENT_ALERT_TEMPLATE_NAME", "")
AGENT_ALERT_TEMPLATE_LANG = os.getenv(
    "AGENT_ALERT_TEMPLATE_LANG",
    "en",
)

# LEVEL 1 scope switch.
#   true  -> price / rent / "how much" are intercepted by the GFI
#            and answered with the standard policy block.
#   false -> those three fall through to the verified price for
#            the unit in context. Everything else still triggers
#            the GFI. Flip this if clients complain the bot will
#            not quote a price.
ENABLE_GFI_PRICE_INTERCEPT = (
    os.getenv("ENABLE_GFI_PRICE_INTERCEPT", "false").lower() == "true"
)
...
# A card must never be folded in half by WhatsApp's own "Read more".
# format_property_results emits this marker between building cards and
# split_whatsapp_message turns each segment into its own message.
MESSAGE_BREAK = "\u0000PANDA_MSG_BREAK\u0000"
...
# Defaults to TRUE. Booked units reaching a client is the one failure
# that costs a relationship, so hiding them must not depend on an
# environment variable surviving every future redeploy. Set the var to
# "false" only to deliberately show everything.
DEFAULT_AVAILABLE_ONLY = (
    os.getenv("DEFAULT_AVAILABLE_ONLY", "true").lower() == "true"
)
...
PERSISTABLE_FILTER_KEYS = {
    "location",
    "building",
    # Set only when the client named a building CODE. Keeps B8 and
    # B97 separable even though both are called "BARAHA".
    "building_code",
    "unit_type",
    "available_only",
    "min_price",
...
def extract_budget_from_text(text: str) -> Dict[str, float]:
    ...
    # A bare figure is how most clients actually write a budget:
    # "1 bedroom 45k", "studio 40k". Without this the amount was dropped
    # and a 45k client was shown a 70k unit. Deliberately narrow: it must
    # carry a k/m suffix or be a realistic annual rent, and it must not
    # be a door number ("unit 402") or an area suffix ("warsan 4").
    if "max_price" not in result and "min_price" not in result:
        # Scan every figure, not just the first: in "1 bedroom 45k" the
        # first is the bedroom count, and re.search would stop there.
        for bare in re.finditer(
            rf"(?<!unit )(?<!unit no )(?<!flat )(?<!no )"
            rf"(?<!\bapt )\b({money})\b",
            norm,
        ):
            token = bare.group(1).strip()
            value = parse_money_value(token)

            has_suffix = bool(
                re.search(
                    r"(?:k|m|mn|mil|million|lakh|lac|crore|cr)$",
                    token,
                )
            )
            ...
            if (
                value is not None
                and (has_suffix or value >= 10000)
                and 5000 <= value <= 5000000
            ):
                result["max_price"] = value
                break
...
def extract_filters_from_text(
    ...
    # An explicit building CODE is always recorded, even when the sheet
    # already resolved the NAME. Without this, typing "B8" while the
    # sheet knows a building called "BARAHA" silently lost the code and
    # fell back to the merged B8+B97 group.
    explicit_code = resolve_building_code_from_text(user_text)

    if explicit_code:
        filters["building_code"] = explicit_code

    # A bare building code locks context the same way a name does.
    if not filters.get("building") and not filters.get("location"):
        coded_building = resolve_building_from_knowledge(user_text)
...
def edit_distance_within(a: str, b: str, limit: int = 1) -> bool:
    ...
    Clients on phones type "Morr", "mre", "shw all". An exact set
    lookup drops those out of the deterministic engine entirely, and
    the LLM fallback then parrots the previous message -- which is
    exactly what looked like broken pagination in production.
    """
    if a == b:
        return True
...
def matches_short_command(text: str, options: Any) -> bool:
    """
    Exact match first, then a one-edit tolerance for short single-word
    commands only. Multi-word phrases still require an exact hit, so
    tolerance can never swallow a real sentence.
    """
    ...
    normalized_options = {searchable_text(item) for item in options}
    normalized_options.discard("")

    if normalized in normalized_options:
        return True

    # One short token only -- never a phrase.
    if len(normalized.split()) != 1 or not 3 <= len(normalized) <= 12:
        return False
    ...
    for option in normalized_options:
        ...
        if edit_distance_within(normalized, option, 1):
            return True
...
def is_show_all_reset_request(text: str) -> bool:
    ...
    if matches_short_command(
        norm,
        [
            "everything", "portfolio", "reset", "restart",
            "menu", "options", "home", "back",
        ],
    ):
        return True

    if re.search(
        r"\b(?:main|full|whole)\s+(?:menu|list|portfolio)\b"
        r"|\bgo\s+back\b|\bother\s+areas?\b",
        norm,
    ):
        return True

    # Handles the real-world phrasings that carry an explicit
    # "clear what we were doing" intent.
    if re.search(
        r"\b(?:show|see|list|send|give)\s+(?:me\s+)?"
        r"(?:all|every|the\s+whole|entire)\b"
        r"|\ball\s+(?:options|buildings|areas|properties)\b"
        r"|\bother\s+(?:areas|buildings|options)\b"
        r"|\bstart\s+(?:over|again)\b|\bfrom\s+scratch\b",
        norm,
    ):
        return True
...
def is_more_properties_request(text: str) -> bool:
    """Typo-tolerant: "morr", "mre", "nex" all count as MORE."""
    return matches_short_command(text, MORE_PROPERTY_REQUESTS)
...
def looks_like_followup(text: str) -> bool:
    ...
    # A global command is the OPPOSITE of a follow-up. Returning
    # True here made build_effective_filters inherit the very area
    # the client was asking to escape -- that is why "show all
    # options" kept repeating the same result.
...
def unit_type_matches_record(
    ...
    # When the sheet HAS a unit type column and the cell is filled, that
    # cell is the answer. Blending in row_search_text let a building-level
    # blurb decide a unit's type: "Offers studio, 1BR, and 2BR apartments"
    # sitting in Key Highlights made 14 studios answer a 2BR search. The
    # whole-row blob stays as a fallback only for sheets with no such
    # column, which is what it was written for.
    declared_type = (
    ...
    if declared_type:
        text_parts = [declared_type]
    else:
        text_parts = [row_search_text(record)]
...
def derive_area_for_building(
    ...
    # LOCATION holds only DUBAI / SHARJAH on the live sheet. Using it
    # made the pivot promise alternatives "right next door" that were
    # a different emirate-wide guess - GHUWAIS (Al Qusais) was answered
    # with Burjuman, Jumeirah and Deira stock. Location Keywords is the
    # only column that carries a real neighbourhood.
    for record in records:
        if phrase_in_text(
            building,
            record.get(building_column, ""),
        ):
            area, _metro = split_area_and_metro(
                get_record_value_by_field(
                    record,
                    schema,
                    "landmark_keywords",
            ...
            if (
                location
                and searchable_text(location) not in GENERIC_AREA_TOKENS
            ):
                return location
...
# ============================================================
# v16 DISPLAY HELPERS
# Everything a client actually reads passes through here.
# ============================================================

# Values that mean "no data" but are truthy in Python, which is why
# "Actual Price: -" was reaching clients on 42 of 119 live units.
NULL_TOKENS = {
    "", "-", "--", "---", "n/a", "na", "nil", "none", "null",
    "tbd", "tba", "?", "0",
}

# LOCATION holds only DUBAI / SHARJAH on the live sheet, so it is
# useless as an area label and must never be shown as one.
GENERIC_AREA_TOKENS = {
    "dubai", "sharjah", "ajman", "uae", "united arab emirates",
}

# The building icon on every card. One place to change it.
#   \U0001F3E2  office block   \U0001F3D9  skyline
#   \U0001F3E0  house          \U0001F306  city at dusk
BUILDING_ICON = "\U0001F3E2"

UNIT_TYPE_DISPLAY = {
    "stu": ("\U0001F6CB\uFE0F", "Studio"),
    "studio": ("\U0001F6CB\uFE0F", "Studio"),
    "1br": ("\U0001F6CF\uFE0F", "1 Bedroom"),
    "11br": ("\U0001F6CF\uFE0F", "1+1 Bedroom"),
    "2br": ("\U0001F3E0", "2 Bedroom"),
    "3br": ("\U0001F3E1", "3 Bedroom"),
    "4br": ("\U0001F3E1", "4 Bedroom"),
    "soff": ("\U0001F4BC", "Office"),
    "off": ("\U0001F4BC", "Office"),
    "office": ("\U0001F4BC", "Office"),
    "shp": ("\U0001F3EA", "Shop"),
    "shop": ("\U0001F3EA", "Shop"),
}
...
def is_null_token(value: Any) -> bool:
    """True when a cell is present but carries no usable information."""
    return searchable_text(clean_cell(value)) in NULL_TOKENS


def tidy_number(value: Any) -> str:
    """
    '2209.0' -> '2209'.

    The sheet is read through pandas, which promotes whole numbers to
    floats. Without this, 95 of 119 live units showed as 'Unit 2209.0'.
    """
    text = clean_cell(value)
...
def parse_money(value: Any) -> Optional[int]:
    """
    Reads every rent format present on the live sheet and returns whole
    dirhams, or None when the cell carries no real figure.

        '45K' -> 45000      '43k' -> 43000       '150k' -> 150000
        '63k/65K' -> 63000  (a range: the client is quoted the lower)
        '-' -> None         '' -> None           '0' -> None
    """
    text = clean_cell(value)
    ...
    # A slash means two figures in one cell. Quote the lower one; the
    # higher is an internal note, not an offer.
    for separator in ("/", "\\", " to ", "-"):
        if separator in text and not text.strip().startswith("-"):
    ...
    match = re.match(r"^(\d+(?:\.\d+)?)\s*k$", normalized)
    ...
    if match:
        ...
        # A bare '45' on a rent column means 45,000, never 45 dirhams.
        return int(round(amount * 1000)) if amount < 1000 else int(round(amount))

    return None


def format_money(amount: Optional[int]) -> str:
    return f"AED {amount:,}" if amount else ""


def format_unit_type_display(raw_value: Any) -> Tuple[str, str]:
    """'STU' -> emoji + 'Studio'. Codes are internal; clients read words."""
    text = clean_cell(raw_value)

    if not text:
        return "", ""

    key = searchable_text(text).replace(" ", "")
    emoji, label = UNIT_TYPE_DISPLAY.get(key, ("", ""))

    if label:
        return emoji, label

    # Unknown code: title-case it rather than invent a meaning for it.
    return "\U0001F3E2", text.title()


def split_location_keywords(raw_value: Any) -> List[str]:
    parts = re.split(
        r"\s*(?:,|/|;|\||\n|\r| - | \u2013 | \u2014 )\s*",
        clean_cell(raw_value),
    )
...
def split_area_and_metro(raw_keywords: Any) -> Tuple[str, str]:
    """
    Location Keywords exists so the search engine can match loosely; it
    was never meant to be printed whole. One area and one metro line is
    what a client can actually use.
    """
    parts = split_location_keywords(raw_keywords)
    ...
    for part in parts:
        ...
        if not metro and ("metro" in lowered or "station" in lowered):
            metro = part

        elif not area and "metro" not in lowered:
            area = part

    if not area and parts:
        area = parts[0]

    return area, metro


def resolve_area_label(record, schema) -> Tuple[str, str]:
    """Area and metro for one card, never falling back to 'DUBAI'."""
    area, metro = split_area_and_metro(
        get_record_value_by_field(record, schema, "landmark_keywords")
    )
    ...
    if not area:
        ...
        if searchable_text(fallback) not in GENERIC_AREA_TOKENS:
            area = fallback
...
def build_unit_block(record, schema) -> str:
    """
    One unit, Concierge layout. Every line is omitted rather than
    printed empty, so a thin record still reads as a finished card.
    """
    emoji, type_label = format_unit_type_display(
        get_record_value_by_field(record, schema, "unit_type")
    )
    ...
    if type_label:
        heading_bits.append(
            f"{emoji} *{type_label.upper()}*" if emoji else f"*{type_label.upper()}*"
        )

    if unit_no:
        heading_bits.append(f"Unit {unit_no}")

    lines = ["\u258C" + "  \u00b7  ".join(heading_bits)] if heading_bits else []
    lines.append("\u258C")

    size_raw = tidy_number(
        get_record_value_by_field(record, schema, "size")
    )

    if size_raw and not is_null_token(size_raw):
        size_text = format_size_for_display(size_raw)

        if size_text:
            try:
                size_text = f"{int(float(size_raw)):,} Sq.Ft."
            except (TypeError, ValueError):
                pass

            lines.append(f"\u258C \U0001F4D0 Size        {size_text}")
    ...
    # A "discount" that is not a discount is worse than no discount.
    if listed and best and best >= listed:
        listed = None

    # One price only. A struck-out "Listed" invites the client to open the
    # negotiation below the offer price, and "You Save" is the broker
    # arguing his own case before the client has asked. The offer stands
    # on its own; Mr. Zahid negotiates in person.
    if best or listed:
        lines.append(
            f"\u258C \U0001F48E Your Price  *{format_money(best or listed)}*"
        )

    else:
        # Never invent a number, and never print a dash at a client.
        lines.append(
            f"\u258C \U0001F4AC Price       _on request from {AGENT_NAME}_"
        )

    return "\n".join(lines)


def build_building_card(building_name, group_records, schema) -> str:
    """One building = one WhatsApp message."""
    area, metro = resolve_area_label(group_records[0], schema)

    name = clean_cell(building_name).upper()
    suffix = format_building_display_name(building_name)[len(clean_cell(building_name)):]

    header = [f"{BUILDING_ICON} *{name}*{suffix}"]

    if area:
        header.append(f"\U0001F4CD {area}")

    if metro:
        # "Near" makes it a distance, not an address. Without it the line
        # reads as though the building IS the station.
        lead = "" if metro.lower().startswith("near") else "Near "
        header.append(f"\U0001F687 {lead}{metro}")

    header.append("\u2501" * 16)

    blocks = [
        build_unit_block(record, schema)
...
def format_property_results(
    ...
    """
    Returns one string containing MESSAGE_BREAK markers. The send layer
    turns each segment into its own WhatsApp message, so a card is never
    cut in half by WhatsApp's own "Read more" fold, which was happening
    on every area search measured (871-1325 chars against a ~700 fold).
    """
    ...
    if intro_override:
    ...
    elif filters.get("exclusive_in_area") and filters.get("building"):
        intro = (
            f"Since *{filters['building']}* is our exclusive building "
            f"in *{filters['exclusive_in_area']}*, here is what we "
            "have available:"
        )
        ...
        area_buildings.discard("")

        if len(area_buildings) == 1:
            only_building = clean_cell(
                get_record_value_by_field(records[0], schema, "building")
            )
            intro = (
                f"Since *{only_building}* is our exclusive "
                f"building in *{filters['location']}*, here is what "
                "we have available:"
            )
        else:
            intro = (
    ...
    for index, (building_name, group_records) in enumerate(grouped.items()):
        card = build_building_card(building_name, group_records, schema)

        if index == 0 and not suppress_intro:
            card = intro + "\n\n" + card
    ...
    if video_links:
        if len(video_links) == 1:
            closing.append(
                "\U0001F3A5 I have a layout video for this property "
                "type.\n\U0001F449 *Reply 'YES' and I will send it over.*"
            )
        else:
            closing.append(
                "\U0001F3A5 I have layout videos for these property "
                "types.\n\U0001F449 *Reply 'YES' and I will send them over.*"
            )

    if capped:
        closing.append(
            "\U0001F31F These are the best options I have handpicked "
            "for you right now."
        )
        ...
        if has_more:
            closing.append(
                "\U0001F449 *Reply 'MORE' for other options in this "
                "building.*"
            )
        else:
            closing.append(
                "\U0001F31F These are the best options I have "
                "handpicked for you right now."
            )

    else:
        closing.append(
            "\U0001F449 *Reply with a building name above to see all "
            "its available apartments.*"
        )
    ...
    if tone == TONE_CLOSER:
        closing.append(
            pick_variant(tone_sender, "reassurance", CLOSER_REASSURANCE)
        )
        closing.append(
    ...
    return MESSAGE_BREAK.join(segments)
...
PROCESS_TOPIC_PATTERNS = [
    (
        # "How to book" used to match nothing here, then looks_like_followup
        # saw the word "book" and re-rendered the same listing the client
        # had just read. It is a question about the process and gets an
        # answer about the process.
        "booking",
        re.compile(
            r"\bhow\s+(?:to|do\s+i|can\s+i|does\s+it)\s+"
            r"(?:book|rent|apply|proceed|start|work)\b"
            r"|\bbooking\s+process\b|\brental\s+process\b"
            r"|\bwhat\s+is\s+the\s+process\b|\bwhats\s+the\s+process\b"
            r"|\bnext\s+steps?\b|\bhow\s+it\s+works\b"
            r"|\bwhat\s+(?:do\s+i|documents?)\s+need\b"
            r"|\bpaperwork\b|\bformalit\w*\b"
        ),
    ),
    (
        "payment",
    ...
    (
        ...
        re.compile(
            ...
            r"|\bbroker(?:age)?\s+fee\w*\b"
            r"|\badmin\s+(?:fee|charge)\w*\b|\badmin\b"
            r"|\bcharges?\b"
...
def find_knowledge_snippets(
    ...
    for label, value in flatten_knowledge(knowledge):
        ...
        # A token that matches the key name itself ("ejari") beats
        # any amount of incidental body-text overlap.
        label_key = searchable_text(label)

        if any(token in label_key for token in query_tokens):
            overlap += 2.0
...
def booking_process_answer(sender: str = "") -> str:
    """
    The one question every serious client asks, and the one the bot had no
    answer for. Every figure is read from knowledge.json global_policies -
    nothing here is written into the code, so it stays true when the
    policies change.
    """
    policies = (get_knowledge() or {}).get("global_policies", {}) or {}

    steps = [
        "1\uFE0F\u20E3 Tell me the unit you want and I will arrange a "
        f"viewing with *{AGENT_NAME}*.",
        "2\uFE0F\u20E3 Once you are happy, he agrees the final rent with "
        "the owner.",
        "3\uFE0F\u20E3 You sign the contract and hand over the cheques.",
        "4\uFE0F\u20E3 Ejari is registered and the keys are yours.",
    ]

    money_lines = []

    for key, label in (
        ("cheques_payment", "Cheques"),
        ("security_deposit", "Security deposit"),
        ("commission", "Agency commission"),
        ("ejari", "Ejari"),
        ("move_in_timeline", "Move-in"),
    ):
        value = clean_cell(policies.get(key, ""))

        if value:
            money_lines.append(f"\u2022 *{label}:* {value}")

    if sender:
        set_last_offer(sender, "handoff")

    body = "Here is exactly how it works \U0001F447\n\n" + "\n".join(steps)

    if money_lines:
        body += "\n\n*What it costs:*\n" + "\n".join(money_lines)

    body += (
        f"\n\n*{AGENT_NAME}* ({AGENT_PHONE}) handles the paperwork and "
        "the negotiation himself.\n\nShall I have him call you?"
    )
...
def answer_process_question(
    ...
    def confirm(value: str, label_note: str = "") -> str:
        # Ends with a broker offer, so arm LEVEL 2.
        if sender:
            set_last_offer(sender, "handoff")

        # "Is it chiller free?" answered with "Yes - Chiller: Empower"
        # tells the client the opposite of the truth. A provider name
        # means a separate cooling bill, so the honest answer is No.
        if (
            asks_if_included_free(user_question)
            and COOLING_PROVIDER_PATTERN.search(value or "")
        ):
            provider = COOLING_PROVIDER_PATTERN.search(value).group(0)

            return (
                f"No — {scope_label or 'this building'} is not "
                "chiller free. Cooling is billed separately through "
                f"*{provider.title()}*.\n\n"
                f"*{AGENT_NAME}* ({AGENT_PHONE}) can give you the "
                "typical monthly figure for the unit you like. "
                "Shall I have him share it?"
            )
    ...
    def unresolved() -> str:
        """Data exists but says "to be confirmed" -- that is a pivot."""
        if sender:
            set_last_offer(sender, "handoff")

        return unverified_detail_pivot(
            f"{detail_label}{scope_note}" if scope_label
            else detail_label
        )
    ...
    if active_building:
        entry = resolve_building_knowledge(
            active_building,
            clean_cell(active_context.get("building_code", "")),
        )

        # When one name covers several buildings, say so plainly.
        merged_codes = (entry or {}).get("merged_codes") or []

        if len(merged_codes) > 1:
            scope_note = (
                f" for *{scope_label}* "
                f"({" and ".join(merged_codes)})"
            )
    ...
    # ---- STEP B: only an area is locked --------------------------
        ...
        for name in buildings_in_area(
            ...
            # An unconfirmed value is not a data point; excluding it
            # stops it from faking a disagreement across the area.
            if value and classify_attribute_value(
                value,
                keywords,
            ) != "unknown":
    ...
    policy_label, policy_value = lookup_global_policy(
        topic,
        user_question,
    )

    if policy_value and classify_attribute_value(
        policy_value,
        keywords,
    ) == "unknown":
        return unresolved()
    ...
    if policy_value:
        ...
        return (
            f"*{policy_label}:* {policy_value}"
            + get_disclaimer_line()
            + f"\n\nShall I have *{AGENT_NAME}* ({AGENT_PHONE}) "
            "confirm this for your unit?"
        )

    # Loose snippet matching is a GLOBAL fallback. With a building in
    # context it answered a parking question about AL FAHIDI - a
    # building absent from knowledge.json - with the commission rate and
    # the disclaimer paragraph. Inside a building scope, "I do not hold
    # that detail" is the only honest answer, so fall through to the
    # targeted pivot below.
    snippets = (
        []
        if scope_label
        else find_knowledge_snippets(
...
def remember_context_filters(
    sender: str,
    filters: Dict[str, Any],
) -> None:
    """
    Locks building/area context as soon as it is RECOGNISED.

    update_session only persists filters on a matched turn, so a
    building that exists in knowledge.json but has no sheet rows was
    silently losing its context and breaking the lock on the next
    message.
    """
    building = clean_cell((filters or {}).get("building", ""))
    location = clean_cell((filters or {}).get("location", ""))
    ...
    with sessions_lock:
        ...
        state = session.setdefault("state", {})
        last = dict(state.get("last_filters", {}) or {})

        # A newly named building replaces an area, and vice versa.
        if building:
            last.pop("location", None)
            last["building"] = building

            # Re-set every time: naming the building without a code
            # must clear a code left over from an earlier turn.
            building_code = clean_cell(
                (filters or {}).get("building_code", "")
            )

            if building_code:
                last["building_code"] = building_code
            else:
                last.pop("building_code", None)
        else:
            last.pop("building", None)
            last.pop("building_code", None)
            last["location"] = location

        state["last_filters"] = last
...
# The old set promised an outcome ("absolutely no issues living here")
# and leaned on scarcity ("rents out very fast", "moves quickly") - the
# urgency language that was deliberately removed from the spec but had
# stayed in the code. Nothing here claims anything that cannot be shown.
CLOSER_REASSURANCE = [
    "Happy to hold your questions with the owner before you decide "
    "anything.",
    "Take your time with it — nothing needs deciding today.",
    "If you want to compare it against another building first, just "
    "say the word.",
    "I can arrange a viewing whenever suits you, evenings included.",
]
...
BUILDING_KNOWLEDGE_FIELD_ALIASES = {
    ...
    "fees": [
        # admin_charge is the real key in knowledge.json. The
        # singular form must be listed or C8 and D10 never match.
        "admin charge", "admin charges", "admin fee", "admin",
        "commission", "agency fee", "broker fee",
        "security deposit", "deposit", "service charge",
        "maintenance", "charge", "charges", "fees", "extra charges",
    ],
    "contract": [
...
def build_building_knowledge_index() -> Dict[str, Any]:
    ...
    index: Dict[str, Any] = {
        "by_code": {},
        "by_name": {},
        # Two buildings can share a name (B8 and B97 are both
        # "BARAHA"). by_name keeps the first for compatibility;
        # by_name_group keeps them all so a name query can answer
        # for every building that carries it.
        "by_name_group": {},
        "entries": [],
        "global_data": {},
    }
    ...
    if isinstance(raw_buildings, dict):
        for code, entry in raw_buildings.items():
            ...
            if name_key:
                index["by_name"].setdefault(name_key, record)
                index["by_name_group"].setdefault(
                    name_key,
                    [],
                ).append(record)
...
def merge_building_entries(
    entries: List[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """
    Folds buildings that share a name into one combined entity.

    Fields every building agrees on collapse to a single value. Fields
    that differ are kept per code ("B8: X | B97: Y") so the client is
    never told one building's rule as if it covered both.
    """
    if not entries:
        return None
    ...
    for key in field_keys:
        if key in {"name", "code", "building", "title"}:
            continue

        pairs: List[Tuple[str, str]] = []

        for entry in entries:
            value = entry.get("fields", {}).get(key)
        ...
        distinct = {searchable_text(value) for _, value in pairs}

        if len(distinct) == 1:
            merged_fields[key] = pairs[0][1]
        else:
            merged_fields[key] = " | ".join(
                f"{code}: {value}" for code, value in pairs
            )
    ...
    return {
        "code": " / ".join(codes),
        "name": clean_cell(entries[0].get("name", "")),
...
def resolve_building_from_knowledge(text: str) -> str:
    """
    Resolves a building the client named, from knowledge.json alone.

    Necessary because the portfolio in knowledge.json can be wider
    than the live sheet: a building with no current listings must
    still be able to hold the context lock.
    """
    ...
    # Codes first -- word-boundary matched so "C8" does not fire
    # inside a unit number like "C8-201".
    for code_key, record in index["by_code"].items():
        if len(code_key) < 2:
            continue

        if re.search(rf"\b{re.escape(code_key)}\b", normalized):
            return (
                clean_cell(record.get("name", ""))
                or clean_cell(record.get("code", ""))
            )

    # Then names, longest match wins so "AL HASEEN 2" is never
    # swallowed by "AL HASEEN".
    best = ""
    best_length = 0

    for name_key, record in index["by_name"].items():
        if len(name_key) < 3:
            continue

        if re.search(rf"\b{re.escape(name_key)}\b", normalized):
            if len(name_key) > best_length:
                best = clean_cell(record.get("name", ""))
                best_length = len(name_key)

    return best


def resolve_building_code_from_text(text: str) -> str:
    """Returns the CODE the client typed, or "" if they used a name."""
    normalized = normalize_knowledge_key(text)
    ...
    for code_key, record in index["by_code"].items():
        ...
        if re.search(rf"\b{re.escape(code_key)}\b", normalized):
            return clean_cell(record.get("code", ""))
...
def resolve_building_knowledge(
    label: str,
    building_code: str = "",
) -> Optional[Dict[str, Any]]:
    """
    Finds the knowledge entry for a building.

    A CODE always wins and always resolves to exactly one building,
    so B8 and B97 stay separable. A NAME resolves to every building
    carrying it, merged into one combined entity.
    """
    index = build_building_knowledge_index()

    # An explicit code pins one building, never the group.
    code_key = normalize_knowledge_key(building_code)

    if code_key:
        pinned = index["by_code"].get(code_key)
    ...
    group = index["by_name_group"].get(key)

    if group:
        return merge_building_entries(group)
        ...
        for candidate in (record["name"], record["code"]):
            ...
            if (
                ...
                if len(candidate_key) > best_length:
                    ...
                    best_name = (
                        candidate_key
                        if candidate_key == normalize_knowledge_key(
                            record["name"]
                        )
                        else ""
                    )

    # A fuzzy NAME hit also merges its group.
    if best_name:
        group = index["by_name_group"].get(best_name)
...
def get_active_context(
    ...
    for source in (filters or {}, previous_filters):
        building = clean_cell(source.get("building", ""))

        # Paired deliberately: a building and its code must come
        # from the SAME source, or a stale code could pin the wrong
        # half of a shared-name pair.
        if building and "building" not in context:
            context["building"] = building
            context["building_code"] = clean_cell(
                source.get("building_code", "")
            )

        for key in ("location", "unit_no"):
...
# District cooling providers. Naming one of these is the OPPOSITE of
# "chiller free" - the tenant gets a separate cooling bill - yet the
# generic classifier read "Empower" as neutral info and the bot replied
# "Yes - confirmed: Chiller: Empower" to "is it chiller free?".
COOLING_PROVIDER_PATTERN = re.compile(
    r"\bempower\b|\bdewa\b|\bemicool\b|\btabreed\b"
    r"|\bdistrict\s+cooling\b|\bpalm\s+district\b",
    re.IGNORECASE,
)

FREE_QUESTION_PATTERN = re.compile(
    r"\bfree\b|\bincluded\b|\binclusive\b|\bmuft\b|\bfree\s*hai\b",
    re.IGNORECASE,
)


def asks_if_included_free(text: str) -> bool:
    """True for 'is it chiller free', 'chiller free hai kya', 'included?'."""
    return bool(FREE_QUESTION_PATTERN.search(normalize_text(text or "")))
...
# Values that look like data but are really a promise to check
# later. Reporting these as confirmed produces the absurd
# "Yes - confirmed: Parking: To be confirmed".
UNCONFIRMED_VALUE_PATTERN = re.compile(
    r"\bto\s+be\s+confirmed\b|\btbc\b|\btbd\b"
    r"|\bwill\s+be\s+confirmed\b|\bnot\s+(?:yet\s+)?confirmed\b"
    r"|\bexact\s+amount\s+to\s+be\b|\bcheck\s+with\b"
    r"|\bconfirm\s+per\s+building\b|\bask\s+agent\b"
)
...
def classify_attribute_value(
    ...
    # A merged shared-name value looks like "B8: X | B97: Y".
    # Classify each half; a mixed result is plain info so both are
    # shown rather than one verdict being applied to both.
    if " | " in value and re.search(r"^\s*\S+\s*:\s*\S", value):
        verdicts = set()
        ...
        for part in value.split(" | "):
            ...
            if clean_cell(tail):
                verdicts.add(
                    classify_attribute_value(tail, keywords)
                )
    ...
    # Checked before the negative patterns: "exact amount to be
    # confirmed" is unknown, not a refusal.
    if UNCONFIRMED_VALUE_PATTERN.search(norm):
        return "unknown"
...
# ============================================================
# LEVEL 1 — GLOBAL FINANCIAL INTERCEPTOR (GFI)
# ============================================================
#
# Runs before context checks and before listing logic. "payment
# options" must never reach the units list.
#
# Every keyword is matched on a WORD BOUNDARY. A substring test here
# would be a live bug: "rent" sits inside "current", "different" and
# "parent"; "pay" sits inside "repay".
# ============================================================

GFI_CORE_KEYWORDS = [
    "payment", "pay", "cheque", "cheques", "commission",
    "deposit", "fees",
]

# Split out so it can be switched off without disabling the GFI.
GFI_PRICE_KEYWORDS = ["rent", "price", "how much"]


def build_gfi_pattern() -> "re.Pattern":
    keywords = list(GFI_CORE_KEYWORDS)

    if ENABLE_GFI_PRICE_INTERCEPT:
        keywords += GFI_PRICE_KEYWORDS
...
def is_financial_query(text: str) -> bool:
    ...
    return bool(GFI_PATTERN.search(norm))


# Defaults used when knowledge.json carries no global_policies block.
DEFAULT_GLOBAL_POLICIES = {
    "commission": "5% for residential, 10% for commercial.",
    "security_deposit": (
        "5% for unfurnished, 10% for furnished/commercial "
        "(Refundable)."
    ),
    "payment_plan": (
        "Flexible options, typically 4 to 6 cheques."
    ),
}

GLOBAL_POLICY_KEY_ALIASES = {
    "commission": ["commission", "commission policy", "agency commission"],
    "security_deposit": [
        "security deposit", "deposit", "security_deposit",
    ],
    "payment_plan": [
        # cheques_payment is the real key in knowledge.json.
        "cheques payment", "payment cheques", "payment plan",
        "payment_plan", "payment terms", "cheques", "cheque",
    ],
}


def get_global_policies() -> Dict[str, str]:
    """
    Pulls the three standard policies from knowledge.json
    global_policies (or company_policy / policies), falling back to the
    approved defaults so the reply is never blank.
    """
    index = build_building_knowledge_index()
    global_data = index.get("global_data", {}) or {}

    # Flatten any of the accepted container names into one lookup.
    pool: Dict[str, Any] = {}
    ...
    for container in (
        ...
        section = global_data.get(container)

        if isinstance(section, dict):
            for key, value in section.items():
                pool.setdefault(normalize_knowledge_key(key), value)
    ...
    for field, aliases in GLOBAL_POLICY_KEY_ALIASES.items():
        found = ""

        for alias in aliases:
            candidate = pool.get(normalize_knowledge_key(alias))
        ...
        resolved[field] = found or DEFAULT_GLOBAL_POLICIES[field]

    return resolved


# Topic -> the global_policies keys that answer it. Deterministic and
# O(1); fuzzy token overlap could not reach short keys like "ejari".
TOPIC_TO_GLOBAL_POLICY_KEYS = {
    "contract": ["ejari", "contract", "tenancy", "lease"],
    "fees": [
        "commission", "security_deposit", "admin_charge", "deposit",
    ],
    "payment": [
        "cheques_payment", "payment_plan", "payment", "cheques",
    ],
    "furnishing": ["furnishing", "furnished"],
    "utilities": ["chiller", "gas", "dewa", "utilities"],
    "occupancy": ["pets", "pet_policy"],
    "handover": ["move_in_timeline", "handover", "move_in"],
    "features": ["maintenance", "amenities", "features"],
    "parking": ["parking"],
...
def get_global_policy_pool() -> Dict[str, str]:
    """Flattens every company-wide policy into one normalised map."""
    index = build_building_knowledge_index()
    global_data = index.get("global_data", {}) or {}
    pool: Dict[str, str] = {}
    ...
    for key, value in global_data.items():
        if isinstance(value, (str, int, float)):
            ...
            if text:
                pool.setdefault(normalize_knowledge_key(key), text)

    return pool


def lookup_global_policy(
    topic: str,
    question: str = "",
) -> Tuple[str, str]:
    """
    Company-wide answer for a topic. Narrowed to the concept the
    client actually named, so "commission" never returns the deposit.
    """
    pool = get_global_policy_pool()

    if not pool:
        return "", ""

    all_keys = TOPIC_TO_GLOBAL_POLICY_KEYS.get(topic, [topic])
    normalized_question = normalize_knowledge_key(question)
    specific: List[str] = []
    ...
    if normalized_question:
        specific = [
            key
            for key in all_keys
            if normalize_knowledge_key(key) in normalized_question
            and normalize_knowledge_key(key) not in GENERIC_TOPIC_WORDS
        ]

    def search(keys: List[str]) -> Tuple[str, str]:
        for key in keys:
            value = pool.get(normalize_knowledge_key(key))

            if value:
                return clean_cell(key).replace("_", " ").title(), value

        for key in keys:
            key_norm = normalize_knowledge_key(key)

            if not key_norm:
                continue

            for pool_key, pool_value in pool.items():
                if key_norm in pool_key and pool_value:
                    return (
                        clean_cell(pool_key).replace("_", " ").title(),
                        pool_value,
                    )

        return "", ""

    # Prefer the concept the client named, then fall back to the whole
    # topic. Narrowing must never turn a present answer into a miss.
    if specific:
        label, value = search(specific)

        if value:
            return label, value

    return search(all_keys)


def get_disclaimer_line() -> str:
    """The client's own disclaimer, when knowledge.json carries one."""
    knowledge = get_knowledge() or {}
    text = clean_cell(knowledge.get("disclaimer", ""))

    return f"\n\n_{text}_" if text else ""


def get_welcome_message() -> str:
    """Branded opener, sourced from knowledge.json when present."""
    knowledge = get_knowledge() or {}

    return clean_cell(
        knowledge.get("welcome_message", "")
    ) or WELCOME_MESSAGE


def build_global_policy_reply(sender: str) -> str:
    """LEVEL 1 output. Fixed wording, values from knowledge.json."""
    policies = get_global_policies()

    # An affirmative on the next turn is a lead hand-off, not a search.
    set_last_offer(sender, "handoff")

    return (
        "For the exact rent and best possible price negotiation, "
        f"*{AGENT_NAME}* will confirm with you directly. However, here "
        "are our standard policies:\n\n"
        f"• *Commission:* {policies['commission']}\n"
        f"• *Security Deposit:* {policies['security_deposit']}\n"
        f"• *Payment Plan:* {policies['payment_plan']}\n\n"
        f"Shall I have *{AGENT_NAME}* ({AGENT_PHONE}) reach out to "
        "discuss the exact rent for the unit you like?"
    )


# ============================================================
# LEVEL 2 — LEAD HAND-OFF + BROKER NOTIFICATION
# ============================================================

HANDOFF_CONFIRMATION_WORDS = {
    "yes", "yes please", "yes pls", "ok", "okay", "okey", "sure",
    "haan", "haan ji", "ji", "ji haan", "please", "pls", "yep",
    "yeah", "yup", "go ahead", "sounds good", "definitely",
    "absolutely", "of course", "do it", "connect me", "call me",
    "theek hai", "thik hai", "acha", "naam", "yes sure",
}


def is_handoff_confirmation(
    text: str,
    previous_state: Dict[str, Any],
) -> bool:
    """
    LEVEL 2 trigger: the bot offered the broker, and the client said
    yes. Only fires when a hand-off was genuinely the last thing
    offered, so a stray "ok" never fabricates a lead.
    """
    if (previous_state or {}).get("last_offer") != "handoff":
        return False
    ...
    return token in HANDOFF_CONFIRMATION_WORDS


def notify_agent_new_lead(
    sender: str,
    active_context: Dict[str, str],
) -> bool:
    """
    Silently alerts the broker on WhatsApp. Never raises, and never
    blocks the client's own reply -- a failed alert must not cost us
    the conversation.
    """
    if not ENABLE_LEAD_ALERTS or not AGENT_ALERT_NUMBER:
        return False

    if not META_TOKEN or not PHONE_NUMBER_ID:
        print("Lead alert skipped: META_TOKEN / PHONE_NUMBER_ID unset.")
        return False

    context_label = (
        clean_cell((active_context or {}).get("building", ""))
        or clean_cell((active_context or {}).get("location", ""))
        or "Not specified"
    )
    unit_label = clean_cell((active_context or {}).get("unit_no", ""))

    if unit_label:
        context_label = f"{context_label} (Unit {unit_label})"

    alert_text = (
        "🚨 New Hot Lead Alert!\n"
        f"Client Phone: {clean_cell(sender) or 'Unknown'}\n"
        f"Last Building Context: {context_label}"
    )
    ...
    if AGENT_ALERT_TEMPLATE_NAME:
        # Templates work outside the 24-hour window.
        payload = {
            "messaging_product": "whatsapp",
            "to": AGENT_ALERT_NUMBER,
            "type": "template",
            "template": {
                "name": AGENT_ALERT_TEMPLATE_NAME,
                "language": {"code": AGENT_ALERT_TEMPLATE_LANG},
                "components": [{
                    "type": "body",
                    "parameters": [
                        {
                            "type": "text",
                            "text": clean_cell(sender) or "Unknown",
                        },
                        {"type": "text", "text": context_label},
                    ],
                }],
            },
        }
    ...
    try:
        ...
        if response.status_code >= 400:
            # Error 131047 = outside the 24-hour service window.
            print(
                "Lead alert failed:",
        ...
        print(f"Lead alert delivered for {sender}.")
        return True

    except Exception as error:
        print("Lead alert error:", error)
        traceback.print_exc()
        return False


def handle_lead_handoff(
    sender: str,
    active_context: Dict[str, str],
) -> str:
    """LEVEL 2 action + reply."""
    # Fire and forget: the client's reply must not wait on Meta.
    try:
        threading.Thread(
            target=notify_agent_new_lead,
            args=(sender, dict(active_context or {})),
            daemon=True,
        ).start()
    except Exception:
        traceback.print_exc()
        notify_agent_new_lead(sender, active_context)

    set_last_offer(sender, "")

    return (
        f"Thank you! *{AGENT_NAME}* has been notified and will reach "
        "out to you shortly."
    )


def should_suppress_intro(
    sender: str,
    filters: Dict[str, Any],
) -> bool:
    """
    True when the scene-setting intro has already been shown for this
    exact scope. Any change of building or area earns a fresh intro.
    """
    scope = searchable_text(
        clean_cell(filters.get("building", ""))
        or clean_cell(filters.get("location", ""))
    )
    ...
    with sessions_lock:
        ...
        state = session.setdefault("state", {})
        already = state.get("intro_shown_for", "")
        state["intro_shown_for"] = scope

    return already == scope


def context_has_changed(
    current_filters: Dict[str, Any],
    previous_state: Dict[str, Any],
) -> bool:
    """
    True when this turn names a building or area different from the
    one already active. Purely comparative -- it knows nothing about
    any specific portfolio and adapts to whatever the sheet holds.
    """
    previous_filters = (previous_state or {}).get("last_filters", {}) or {}

    for key in ("building", "location"):
        incoming = searchable_text(current_filters.get(key, ""))

        if incoming and incoming != searchable_text(
            previous_filters.get(key, "")
        ):
            return True
...
def apply_clean_slate(sender: str) -> None:
    """
    Every jump to a new building or area starts fresh.

    Filter inheritance is already blocked upstream, but the pagination
    cursor and the pinned unit are session-wide -- without this, units
    the client saw in the previous area stay suppressed in the new one.
    """
    with sessions_lock:
        session = user_sessions.get(sender)
        ...
        if session:
            ...
            # Drop inherited refinements; the new scope defines its own.
            last = dict(state.get("last_filters", {}) or {})

            for key in ("unit_type", "min_price", "max_price", "unit_no"):
                last.pop(key, None)
...
def distinct_buildings_for_scope(
    records: List[Dict[str, Any]],
    schema: Dict[str, str],
    filters: Dict[str, Any],
) -> List[str]:
    """Distinct building names inside the filtered scope, in sheet order."""
    if not records or not schema.get("building"):
        return []
...
def promote_single_building_area(
    matches: List[Dict[str, Any]],
    schema: Dict[str, str],
    filters: Dict[str, Any],
) -> Dict[str, Any]:
    """
    Area mode exists so a client can compare buildings: one teaser unit
    each. When the area holds exactly ONE building there is nothing to
    compare, and the teaser hides every other unit in it -- "more" then
    reports the list exhausted while real inventory sits unseen.

    So: promote the search to building mode. Fully dynamic; the count
    is measured from the data on every turn.
    """
    if not filters.get("location") or filters.get("building"):
        return filters

    buildings = distinct_buildings_for_scope(matches, schema, filters)

    if len(buildings) != 1:
        return filters

    promoted = dict(filters)
    promoted["building"] = buildings[0]

    # Remembered so the intro can still say "our exclusive building in
    # <area>" rather than the generic building greeting. Not a search
    # filter -- apply_hard_filters ignores keys it does not know.
    promoted["exclusive_in_area"] = clean_cell(filters["location"])

    return promoted


def clear_active_context(sender: str) -> None:
    """
    Hard state wipe for explicit global commands.

    Clears the building, the area, the unit pin, the pending
    qualifier, any armed videos and the pagination cursor, so the next
    search starts genuinely clean.
    """
    with sessions_lock:
        session = user_sessions.get(sender)
    ...
    # Clears the "already shown" cache so the next search can surface
    # units the client has seen before -- they explicitly asked to
    # start over.
    try:
        clear_property_pagination(sender)
    except Exception:
...
def build_portfolio_overview(
    ...
    """
    The global answer to "show me everything": areas and how many
    buildings sit in each. A full unit dump would break the display
    contract and be unreadable on WhatsApp.
    """
    if not records or not schema.get("location"):
        return ""

    # Counting booked-only buildings advertised stock that cannot be
    # shown: 25 buildings claimed against 22 actually available.
    if DEFAULT_AVAILABLE_ONLY:
        records = filter_available_records(records, schema)

        if not records:
            return ""

    areas: "OrderedDict[str, set]" = OrderedDict()
    ...
    for record in records:
        ...
        if building:
            areas[area].add(building)

    if not areas:
        return ""

    lines = [
        "Of course — here is my full portfolio at a glance 👇",
        "",
    ]

    for area, buildings in list(areas.items())[:15]:
        count = len(buildings)
        label = "building" if count == 1 else "buildings"
        lines.append(f"📍 *{area}* — {count} {label}")

    lines.append("")
    lines.append(
        "👉 *Type any area or building name and I will show you "
        "exactly what is available there.*"
    )
...
def route_context_locked_query(
    ...
    # How to book is the same answer in every building, so the context
    # lock has nothing to add. Without this the STEP-A router reached
    # answer_process_question, found no "booking" field on DOTCOM, and
    # pivoted - the client asked a fair question and got deflected.
    if topic == "booking":
        return booking_process_answer(sender)
...
def build_verified_property_fact_reply(
    ...
    if asks_price:
        # Raw sheet strings ("45K", "63k/65K", "-") were being quoted
        # straight at the client here, so the direct price answer looked
        # nothing like the listing card. Same parser, same formatting.
        actual = parse_money(
            unanimous_field_value(matches, schema, "price")
        )
        offer = parse_money(
            unanimous_field_value(matches, schema, "offer_price")
        )

        if actual and offer and offer >= actual:
            actual = None
        ...
        if actual or offer:
            ...
            # The card shows one price now, so this must too. Quoting a
            # struck-out higher figure here invited the client to open the
            # negotiation below the number Mr. Zahid actually needs.
            body = f"\U0001F48E *{format_money(offer or actual)}* per year."
...
def render_property_page(
    ...
    return format_property_results(
        ...
        suppress_intro=(
            not intro_override
            and should_suppress_intro(sender, filters)
        ),
    )
...
# CONVERSATION LAYER
#
# main.py answers a SEARCH at 12/12 and a CONVERSATION at 0/5 on viewing
# requests and 0/5 on "not interested" / "yes interested". The split below
# is by measured strength, not by preference: a message that names a real
# area, building, unit type or budget goes to the engine that gets those
# right; everything else goes to a model that can read a sentence.
#
# Measured on 56 real client messages, the routing signal itself was right
# 55 times, and the one miss ("close to metro please" fuzzy-matching a
# keyword blob) is handled by GENERIC_MATCH_WORDS below.
# ============================================================

# A location match driven only by one of these is an artifact of substring
# matching against the Location Keywords blob, not a client naming a place.
GENERIC_MATCH_WORDS = {
    "metro", "station", "near", "nearby", "close", "area", "building",
    "mall", "park", "hospital", "school", "beach", "exit", "behind",
    "opposite", "walk", "walking", "minute", "minutes",
}


def message_is_a_property_search(
    user_text: str,
    records: List[Dict[str, Any]],
    schema: Dict[str, str],
) -> bool:
    """
    True when the client named something concrete the deterministic engine
    resolves reliably. This is a question about the DATA, not about intent,
    which is why it can be answered without a classifier.
    """
    try:
        filters = extract_filters_from_text(user_text, records, schema)
    except Exception:
        return False
    ...
    if location:
        tokens = {
            t for t in meaningful_tokens(user_text)
            if t not in GENERIC_MATCH_WORDS
        }

        # The matched place has to be traceable to a word the client
        # actually typed. "close to metro please" matched a keyword blob
        # seven items long on the strength of the word "metro" alone.
        #
        # A plain substring test is too strict: "burjman" is a typo for
        # Burjuman and the deterministic engine resolves it correctly
        # 6 times out of 6. Near-matching keeps that working.
        place_words = [
            w for w in searchable_text(location).split() if len(w) > 2
        ]

        def _traceable(token: str) -> bool:
            if token in searchable_text(location):
                return True

            return any(
                difflib.SequenceMatcher(None, token, word).ratio() >= 0.8
                for word in place_words
            )

        if tokens and any(_traceable(t) for t in tokens):
            concrete = True
...
def build_agent_tools(
    ...
    """The same functions the deterministic engine uses, exposed to Claude."""

    def _search(args: Dict[str, Any]) -> str:
        filters: Dict[str, Any] = {"available_only": True}
        ...
        for key, field in (
            ...
            value = clean_cell(args.get(key, ""))

            if value:
                filters[field] = value

        ceiling = args.get("max_price")

        if isinstance(ceiling, (int, float)) and ceiling > 0:
            filters["max_price"] = float(ceiling)

        matches = search_properties(
            user_text=clean_cell(args.get("area") or args.get("building") or ""),
            records=records,
            schema=schema,
            filters=filters,
            fallback_search_text="",
        ...
        if not matches:
            return (
                "No available units match that. Do not invent an "
                "alternative - say plainly that there is nothing and offer "
                "the broker."
            )

        page = matches[: max(1, AREA_MODE_BUILDING_COUNT)]

        card = format_property_results(
            records=page,
            schema=schema,
            columns=columns,
            filters=filters,
        ...
        return (
            "Paste the block below into your reply exactly as it is, "
            "without changing any figure:\n\n"
            + card.replace(MESSAGE_BREAK, "\n\n")
        )

    def _policy(args: Dict[str, Any]) -> str:
        policies = (get_knowledge() or {}).get("global_policies", {}) or {}
        topic = searchable_text(args.get("topic", "")).replace(" ", "_")

        if topic in ("all", "", "everything"):
            return "\n".join(
                f"{k}: {v}" for k, v in policies.items() if v
        ...
        return (
            f"No policy on file for '{args.get('topic')}'. Say so and offer "
            "the broker - do not guess."
        )

    def _building_facts(args: Dict[str, Any]) -> str:
        name = clean_cell(args.get("building", ""))

        if not name:
            return "No building given."

        index = build_building_knowledge_index()
        key = normalize_knowledge_key(name)
        entry = index["by_name"].get(key) or index["by_code"].get(key)
        ...
        if not entry:
            return (
                f"No fact sheet on file for '{name}'. Say so and offer the "
                "broker - do not guess."
            )
        ...
        return "\n".join(
            ...
            if v and k != "name"
        ) or f"The fact sheet for {name} is empty."

    def _callback(args: Dict[str, Any]) -> str:
        reason = clean_cell(args.get("reason", ""))

        try:
            notify_agent_new_lead(sender, reason or "Client asked for a callback")
        except Exception:
            pass
        ...
        return (
            f"Broker notified. Tell the client that {AGENT_NAME} "
            f"({AGENT_PHONE}) will contact them, and confirm what he will "
            "help with. Keep it to two lines."
        )

    return {
        "search_properties": _search,
        "get_policy": _policy,
        "get_building_facts": _building_facts,
...
def try_agent_reply(
    ...
    columns: List[str],
    previous_state: Dict[str, Any],
    past_history: List[Dict[str, str]] = None,
) -> str:
    """
    Returns a verified reply, or "" so the caller continues into the
    deterministic engine untouched. Never raises.
    """
    if panda_agent is None or client is None:
        return ""

    try:
        context = get_active_context({}, previous_state) or {}
        bits = []

        if context.get("building"):
            bits.append(f"The client is looking at {context['building']}.")

        if context.get("location"):
            bits.append(f"Area under discussion: {context['location']}.")

        reply = panda_agent.run_agent(
            user_text=user_question,
            history=past_history or [],
            tool_impls=build_agent_tools(sender, records, schema, columns),
            call_model=client.messages.create,
            system_prompt=panda_agent.build_system_prompt(
        ...
        return clean_cell(reply or "")

    except Exception as error:
        print("agent layer failed, falling back:", error)
        traceback.print_exc()
        return ""
...
def create_ai_reply(
    ...
    # ------------------------------------------------------------
    # LEVEL 0 -- EXPLICIT GLOBAL COMMAND: wipe state, go global
    # ------------------------------------------------------------
    # Must run before the context lock, or the lock will defend the
    # very context the client is asking to leave.
    if is_show_all_reset_request(user_question):
        clear_active_context(sender)

        previous_state = {}
        overview_records, overview_schema, _ = get_properties()
    ...
    # Lock the context the instant it is recognised.
    remember_context_filters(sender, current_filters)
    ...
    # ------------------------------------------------------------
    # LEVEL 0.5 -- CONVERSATION LAYER
    # ------------------------------------------------------------
    # Placed AFTER reset and video consent, which are exact commands and
    # must never be paraphrased, and BEFORE every regex level below. Those
    # levels answer a search at 12/12 and a conversation at 0/5 on viewing
    # requests, so they only get the messages they are good at: ones that
    # name a real area, building, unit type or budget.
    #
    # Everything else - "kal dekh sakta hu", "not interested", "who am i
    # talking to", "55 me dedo" - goes to a model that can read a sentence.
    # If it is unavailable, times out, or writes one figure the tools did
    # not return, this returns "" and the regex levels run exactly as they
    # do today. The bot is never left silent.
    if not message_is_a_property_search(user_question, records, schema):
        agent_reply = try_agent_reply(
            sender=sender,
            user_question=user_question,
            records=records,
            schema=schema,
        ...
        if agent_reply:
            return finish(agent_reply, translate=False)

    # ------------------------------------------------------------
    # LEVEL 1 -- GLOBAL FINANCIAL INTERCEPTOR
    # ------------------------------------------------------------
    # Highest priority. Runs before context and before listings so
    # that "payment options" can never reach the units list.
    if is_financial_query(user_question):
        return finish(
            build_global_policy_reply(sender),
    ...
    # LEVEL 2 -- LEAD HAND-OFF + BROKER NOTIFICATION
    # ------------------------------------------------------------
    if is_handoff_confirmation(user_question, previous_state):
        return finish(
            handle_lead_handoff(sender, active_context),
            filters_used=current_filters,
        )

    # ------------------------------------------------------------
    # LEVEL 3 -- STRICT CONTEXT-LOCKED FEATURE QUERIES
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # LEVEL 4 -- FALLBACK: intents, listings, then the pivot
    # ------------------------------------------------------------
    ...
    if (
        ...
        # CLEAN SLATE: a new building or area wipes inherited
        # refinements AND the pagination cursor, so the new scope
        # starts from unit one with no leftover filters.
        if context_has_changed(current_filters, previous_state):
            apply_clean_slate(sender)

            previous_state = dict(previous_state)
            previous_state["last_filters"] = {
                key: value
                for key, value in (previous_state.get(
                    "last_filters",
                    {},
                ) or {}).items()
                if key in {"building", "location", "building_code"}
            }
    ...
    # build_effective_filters rebuilds from a known key set, so an
    # explicit building CODE has to be re-applied or B8 and B97 collapse
    # back into the merged group.
    if current_filters.get("building_code"):
        effective_filters["building_code"] = current_filters["building_code"]
    ...
    # A bare filler ("ok", "hmm", "?") arrives here with matches = the
    # ENTIRE available inventory, because DEFAULT_AVAILABLE_ONLY puts
    # available_only into every filter set and search_properties then has
    # nothing else to narrow by. Rendering that dump burned 4 of the 7
    # session slots on a client who had asked for nothing, so the next
    # real search reported "these are the best options I have" against a
    # 119-unit inventory. looks_like_followup("ok") is True, which is why
    # only last_search_text is a safe test for genuine continuation.
    unqualified_dump = (
        not has_specific_property_filter(effective_filters)
        and not previous_state.get("last_search_text")
    )

    # "studio under 40k" is the most qualified opener a client can send.
    # Answering it with "which area?" and no stock lost the lead, and
    # when there genuinely was nothing in that budget the client was
    # never told. Show the matching stock, then ask the area question
    # underneath it - the two are not mutually exclusive.
    qualify_after_listing = False

    if missing_location_context and matches:
        missing_location_context = False
        qualify_after_listing = True

    if matches and not missing_location_context and not unqualified_dump:
        # Measured from the data every turn, never hardcoded.
        effective_filters = promote_single_building_area(
            matches,
            schema,
            effective_filters,
        )
        ...
        # The listing answers what they asked; the question narrows the
        # next turn. It rides at the end of the closing segment so it is
        # one message, not a second notification.
        if qualify_after_listing and reply:
            reply = reply + (
                "\n\n\U0001F4CD *Which area or building shall I focus on?* "
                "I will show you everything that fits there."
            )
    ...
    if missing_location_context:
        ...
        # No stock at all for what they asked. Asking "which area?" here
        # sends the client hunting for something that does not exist in
        # any area. Say so, and show the closest real thing instead.
        if not matches and records:
            relaxed = dict(effective_filters)
            relaxed.pop("min_price", None)
            relaxed.pop("max_price", None)

            nearest = search_properties(
                user_text=user_question,
                records=records,
                schema=schema,
                filters=relaxed,
            ...
            if nearest:
                ...
                ceiling = effective_filters.get("max_price")

                intro = (
                    f"I do not have a *{wanted}* "
                    + (
                        f"under {format_money(int(ceiling))} "
                        if ceiling
                        else ""
                    )
                    + "in stock right now — I would rather tell you "
                    "straight than send you looking.\n\n"
                    f"Here is the closest I do have \U0001F447"
                )
...
def split_whatsapp_message(
    ...
    # Explicit breaks win. Each segment is a finished card and is sent
    # as its own message, with no "(1/3)" counter - that counter is for
    # one message that had to be cut, not for a deliberate sequence.
    if MESSAGE_BREAK in raw:
        segments = [
            clean_cell(segment)
            for segment in raw.split(MESSAGE_BREAK)
        ]
        ...
        for segment in segments:
            ...
            if len(segment) <= limit:
                ...
                chunks.extend(split_whatsapp_message(segment, limit))
