...
def format_property_results(
    ...
    if intro_override:
        ...
        # Sole building in its area: name the area so the client knows
        # this IS the whole area, not one option among several.
        intro = (
        ...
        # One building in the area? Saying "buildings" plural and
        # "I would recommend" reads like a list is coming.
        area_buildings = {
            searchable_text(
    ...
    # The scene-setting line belongs to the first turn in a scope.
    # Re-printing it on every follow-up filter is what made the
    # bot look like it had forgotten the conversation.
    lines: List[str] = [] if suppress_intro else [intro]
    grouped: "OrderedDict[str, List[Dict[str, Any]]]" = OrderedDict()
    ...
    # lstrip: with the intro suppressed the list starts with a
    # blank separator, which renders as an empty first line.
    return "\n".join(lines).lstrip("\n")
