...
def build_unit_block(record, schema) -> str:
    ...
    if listed and best:
        lines.append(f"\u258C \U0001F3F7\uFE0F Listed      ~{format_money(listed)}~")
        lines.append(f"\u258C \U0001F48E Your Price  *{format_money(best)}*")

        saving = listed - best

        if saving > 0:
            lines.append(
                f"\u258C \U0001F381 You Save    _{format_money(saving)}_"
            )
...
def build_building_card(building_name, group_records, schema) -> str:
    ...
    header = [f"{BUILDING_ICON} *{clean_cell(building_name).upper()}*"]

    if area:
        header.append(f"\U0001F4CD {area}")

    if metro:
        header.append(f"\U0001F687 {metro}")
...
def build_verified_property_fact_reply(
    ...
    if asks_price:
        ...
        if actual or offer:
            ...
            if actual and offer:
                body = (
                    f"~{format_money(actual)}~ — and the best price "
                    f"right now is \U0001F48E *{format_money(offer)}*."
                )
            else:
                body = f"*{format_money(actual or offer)}*."
