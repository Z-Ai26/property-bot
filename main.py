...
def search_properties(
    ...
    Applies strict schema filtering first, falls back to intelligent text scoring,
    and rigidly enforces anti-spam (Top 4) limits to ensure clean UX.
    """
    if not records:
        return []
    ...
    # SYSTEM RULE: Never output more than 4 records to prevent WhatsApp spam.
    MAX_RESULTS = 4 

    # EDGE CASE: User asked for a unit type within a text-based fallback context
    only_unit_followup = (
    ...
    if specific_filter_present or filters.get("available_only"):
        ...
        if hard_filtered:
            # If we have a definitive location or building match, return the cleanest subset instantly
            if filters.get("location") or filters.get("building"):
                return dedupe_records(hard_filtered)[:MAX_RESULTS]

        # FALLBACK PATH: If hard filters yield 0 results, score the raw text
    ...
    # No specific schema filters detected: execute pure text scoring
