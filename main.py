...
def build_effective_filters(
    ...
    """
    ENTERPRISE-GRADE CONTEXT MANAGER:
    Intelligently merges current filters with session memory using advanced NLP heuristics.
    Instantly drops legacy context if a context-switch (new location/entity) is detected.
    """
    ...
    # --- ADVANCED INTENT DETECTION (Context Switch Heuristics) ---
    norm_text = normalize_text(user_text)

    # 1. Explicit Context Breakers: Words that strongly indicate a shift in location
    # Covers: "in deira", "at burjuman", "near metro", "what about marina", "show me"
    context_breaker_pattern = re.compile(
        r"\b(in|at|near|around|for|what about|how about|show me|look in|search in)\b",
        flags=re.IGNORECASE
    )
    has_context_breaker = bool(context_breaker_pattern.search(norm_text))

    # 2. Strong Entity Detection: Did our extractor find a brand new location/building?
    has_new_entity = bool(current_location or current_building)

    # 3. Follow-up Detection: Is it a short query ("2 br", "studio") or explicit follow-up word?
    tokens = meaningful_tokens(user_text)
    is_short_query = len(tokens) <= 4
    is_explicit_followup = looks_like_followup(user_text)

    # THE VERDICT: Only inherit legacy memory if NO new entity is detected, 
    # NO context breakers are used, AND it genuinely looks like a follow-up.
    inherit_memory = not has_new_entity and not has_context_breaker and (is_short_query or is_explicit_followup)

    # --- FILTER MERGING LOGIC ---
    if current_location:
        # Hard pivot to new location
        effective["location"] = current_location
        if current_building: effective["building"] = current_building
        if current_unit_type: effective["unit_type"] = current_unit_type

    elif current_building:
        # Pivot to building, conditionally keep legacy location ONLY if explicitly continuing
        if previous_filters.get("location") and inherit_memory:
        ...
        # User provided ONLY a unit type (e.g., "1 br", "studio")
        ...
        if previous_filters.get("building") and inherit_memory:
            effective["building"] = previous_filters["building"]
            inherited_context = True

        effective["unit_type"] = current_unit_type

        # If schema didn't catch location before, use previous raw text as fallback
        if not inherited_context and previous_search_text and inherit_memory:
            search_text = previous_search_text

    elif is_show_all_reset_request(user_text) and (previous_filters or previous_search_text):
        if previous_filters.get("location"): effective["location"] = previous_filters["location"]
        if previous_filters.get("building"): effective["building"] = previous_filters["building"]
        search_text = previous_search_text or user_text

    elif inherit_memory and (previous_filters or previous_search_text):
        # Pure follow-up ("price?", "details?")
        effective = {
            k: v for k, v in previous_filters.items()
            if k in {"location", "building", "unit_type", "available_only"} and v
        }
        search_text = previous_search_text or user_text
    ...
    # Apply global availability toggle if enabled
...
def search_properties(
    ...
    """
    SMART HYBRID ENGINE: 
    Prioritizes exact matches. If not found, strictly looks for 'Nearby' cross-sells.
    Kills totally unrelated garbage.
    """
    if not records:
        return []
    ...
    if only_unit_followup:
        ...
        if base_records:
            narrowed = [
                ...
                if unit_type_matches_record(rec, filters["unit_type"], schema)
            ]
            if filters.get("available_only"):
                narrowed = filter_available_records(narrowed, schema)
            if narrowed:
                return dedupe_records(narrowed)[:MAX_RESULTS]
        return []
    ...
    if specific_filter_present or filters.get("available_only"):
        ...
        if hard_filtered:
            ranked_filtered = score_search_records(fallback_search_text, hard_filtered)
            if ranked_filtered:
                return dedupe_records(ranked_filtered)[:MAX_RESULTS]
            return dedupe_records(hard_filtered)[:MAX_RESULTS]

        # 2. SMART CROSS-SELL (The 90% Rule)

        fallback_results = score_search_records(fallback_search_text, records)
        
        target_keywords = []
        if filters.get("location"): target_keywords.append(filters["location"].lower())
        if filters.get("building"): target_keywords.append(filters["building"].lower())
        
        if target_keywords:
            strict_nearby_matches = []
            for rec in fallback_results:

                rec_text = " ".join(str(v).lower() for v in rec.values() if v)
                

                if any(kw in rec_text for kw in target_keywords):
                    strict_nearby_matches.append(rec)
           
            fallback_results = strict_nearby_matches

        if filters.get("unit_type"):
            fallback_results = [
                rec for rec in fallback_results
                if unit_type_matches_record(rec, filters["unit_type"], schema)
            ]
        ...
        return dedupe_records(fallback_results)[:MAX_RESULTS]

    # Pure text search fallback (general queries without specific filters)
    ...
    return dedupe_records(scored_results)[:MAX_RESULTS]
...
def format_property_results(
    ...
    for building_name, group_records in grouped.items():
        ...
        for column in output_columns:
            ...
            # Block empty columns, 'Unnamed', and 'Column_15' type garbage
            if not col_header or "unnamed" in col_lower or "column_" in col_lower or "column " in col_lower:
                continue
...
# ============================================================
# Smart Apology Engine 
# ============================================================
def create_smart_apology(user_question: str, past_history: List[Dict[str, str]], filters: Dict[str, Any]) -> str:
    if not client:
        return "Sorry, I couldn't find an exact match for that right now. Please connect with Mr. Zahid at +971562625777."
    
    filter_desc = describe_filters(filters) if filters else user_question
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "The system just searched our live property database and found ZERO MATCHES (not even nearby) for the user's request. "
                f"Specifically, we have no matches for: {filter_desc}. "
                "Your job is to politely reply to the user. Say something like: "
                "'Sorry, right now we don't have [what they asked for] available.' "
                "Do NOT invent or suggest other random properties. Keep it short, empathetic, and natural. "
                "Always end with: 'For offline inventory or other options, please connect with Mr. Zahid at +971562625777.'"
            )
        }
    ]
    
    if past_history:
        messages.extend(past_history[-4:])
    ...
    try:
        ...
        print(f"Apology AI Error: {e}")
        return f"Sorry, I couldn't find an exact match right now. Please connect with Mr. Zahid at +971562625777."
...
def create_ai_reply(sender: str, user_question: str) -> str:
    ...
    if is_reset_command(user_question):
        ...
        return "Sure — I have reset our chat. Please send me a location, building name, or unit type, and I will check the details."
    ...
    if matches:
        ...
        # 🔥 THE HONEST NOTIFIER (Cross-Sell Logic)
        req_loc = (effective_filters or current_filters).get("location", "")
        if req_loc:
            loc_col = schema.get("location", "")
            if loc_col:
                # Check if the requested location is the MAIN location of the results
                is_exact = any(req_loc.lower() in str(rec.get(loc_col, "")).lower() for rec in matches)
                
                # If it's not the main location, we know it's a "Nearby" match
                if not is_exact:
                    prefix = f"⚠️ _Sorry, we don't have exact availability in *{req_loc.title()}* right now. However, here are some highly relevant nearby options:_\n\n"
                    reply = prefix + reply
    ...
    # Total Miss - Trigger OpenAI Smart Apology
    if is_likely_property_query(user_question, effective_filters or current_filters, previous_state):
        reply = create_smart_apology(
            user_question=user_question, 
            past_history=past_history, 
            filters=effective_filters or current_filters
        )
