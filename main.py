...
def best_value_match(user_text: str, values: List[str], mode: str = "exact") -> str:
    ...
    for value in values:
        ...
        if not value_search or value_search in {"dubai", "uae"}:
            continue
        # -----------------------------
...
def format_property_results(
    ...
    building_column = schema.get("building", "")
    specific_building = filters.get("building")
    ...
    if specific_building:
        ...
        seen_buildings = set()
        unique_building_records = []
        
        for record in records:
            b_name = clean_cell(record.get(building_column, "")) if building_column else "Property"
            if not b_name:
                b_name = "Property"
                
            if b_name not in seen_buildings:
                seen_buildings.add(b_name)
                unique_building_records.append(record)
                
            if len(unique_building_records) >= MAX_PROPERTIES_TO_SHOW:
                break
                
        visible_records = unique_building_records
